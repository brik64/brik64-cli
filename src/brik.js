...
const COMMAND_HELP = {
  ...
  verify: [
    'verify <file.pcd>',
    'Checks a local candidate certificate against the current PCD bytes.',
    'Example:',
  ...
  doctor: [
    'doctor [--json] [--scope <project|all>]',
    'Checks manifest policy, PCD inventory, and PCD parse health.',
    'Example:',
    '  brik64 doctor --scope project --json'
  ],
  engine: [
    'engine status',
    'Inspects the packaged public local runtime profile without exposing internal factory details.',
    'Example:',
    '  brik64 engine status'
  ],
  account: [
    'account status [--json]',
    'Shows local account routing state. Default beta routing remains local unless a managed session is present.',
    'Example:',
    '  brik64 account status --json'
  ],
  telemetry: [
    'telemetry status [--json]',
    'Shows local telemetry preference state.',
    'Example:',
    '  brik64 telemetry status --json'
  ],
  errors: [
    'errors status [--json]',
    'Shows local automatic error reporting preference state.',
    'Example:',
    '  brik64 errors status --json'
  ],
  feedback: [
    'feedback [--message <text>] [--json]',
    'Records local beta feedback for explicit user-submitted review.',
    'Example:',
    '  brik64 feedback --message "emit worked for my project" --json'
  ],
  domain: [
    'domain inspect <file.pcd> [--json]',
    'Reports bounded domain declarations and claim-bearing eligibility for a PCD.',
    'Example:',
    '  brik64 domain inspect pcd/order_gate.pcd --json'
  ],
  adoption: [
    'adoption report [--json] [--out <file>]',
    'Summarizes local lift previews and PCD inventory for migration planning.',
    'Example:',
...
function help(topic) {
  ...
  process.stdout.write('  domain inspect|validate|sheet|add manage bounded domain contracts\n');
  ...
  process.stdout.write('       --type <gate|utility|numeric-monomer|domain-gate> --out <file.pcd>\n');
...
function inferExpressionType(expression, paramTypes) {
  ...
  if (expression.type === 'NumberLiteral') return remember(expression.numericType || (Number.isInteger(expression.value) ? 'i64' : 'f64'));
  if (expression.type === 'ConstLiteral') return remember(expression.literalType || 'i64');
  if (expression.type === 'Identifier') return remember(paramTypes[expression.name] || 'unknown');
  ...
  if (expression.type === 'MonomerCallExpression') {
    ...
    if (expression.boundary === 'declared_boundary_contract' && expression.returnType === 'f64') return remember('f64');
    if (expression.boundary === 'declared_boundary_contract' && expression.returnType === 'bool') return remember('bool');
    if (expression.boundary === 'declared_boundary_contract' && expression.returnType === 'tuple_u8_u8') return remember('tuple_u8_u8');
    if (expression.boundary === 'declared_boundary_contract' && !['f64', 'bool', 'tuple_u8_u8'].includes(expression.returnType)) return remember('i64');
    if (expression.returnType === 'f64') return remember('f64');
    if (expression.returnType === 'bool') return remember('bool');
    if (expression.returnType === 'tuple_u8_u8') return remember('tuple_u8_u8');
    if (expression.operation === 'clamp') {
      const argTypes = expression.args.map((argument) => argument.inferredType || inferExpressionType(argument, paramTypes));
      if (argTypes.includes('f64')) return remember('f64');
    }
    return remember(isNumericType(expression.returnType) || ['u8', 'i8', 'u64'].includes(expression.returnType) ? 'i64' : expression.returnType);
  }
...
function parseDomainBound(value) {
  ...
  fail(65, `pcd_parse_error:unsupported_domain_bound:${trimmed || 'empty'}`);
}

function validateDomainBoundForType(bound, type, name, side) {
  if (bound.kind !== 'literal') return;
  const value = bound.value;
  if (!Number.isFinite(value)) fail(65, `pcd_parse_error:domain_bound_out_of_range:${name}:${side}`);
  if (['i64', 'i32', 'u8', 'u64', 'bool'].includes(type)) {
    if (!Number.isSafeInteger(value)) fail(65, `pcd_parse_error:domain_bound_out_of_range:${name}:${side}`);
  }
  if (type === 'i32' && (value < -2147483648 || value > 2147483647)) {
    fail(65, `pcd_parse_error:domain_bound_out_of_range:${name}:${side}`);
  }
  if (type === 'u8' && (value < 0 || value > 255)) {
    fail(65, `pcd_parse_error:domain_bound_out_of_range:${name}:${side}`);
  }
  if (type === 'bool' && ![0, 1].includes(value)) {
    fail(65, `pcd_parse_error:domain_bound_out_of_range:${name}:${side}`);
  }
}

function removeMatchedSlice(source, match) {
  return `${source.slice(0, match.index)}${source.slice(match.index + match[0].length)}`;
}

function parseDomainContract(pcBody) {
  let executablePcBody = pcBody;
  const domains = [];
  const domainParams = [];
  const invariants = [];
  const conditionalDomains = [];
  const domainParamPattern = /^\s*domain_param\s+([A-Za-z_][A-Za-z0-9_]*)\s*:\s*([A-Za-z0-9_]+)\s*;\s*/m;
  while (true) {
    ...
    executablePcBody = removeMatchedSlice(executablePcBody, match);
  }
  const domainPattern = /^\s*domain\s+([A-Za-z_][A-Za-z0-9_]*)\s*:\s*([A-Za-z0-9_]+)\s*\[\s*([^,\]]+)\s*,\s*([^\]]+)\s*\]\s*;\s*/m;
  while (true) {
    ...
    const max = parseDomainBound(maxRaw);
    validateDomainBoundForType(min, type, name, 'min');
    validateDomainBoundForType(max, type, name, 'max');
    if (min.kind === 'literal' && max.kind === 'literal' && min.value > max.value) fail(65, `pcd_parse_error:domain_bounds_reversed:${name}`);
    domains.push({ name, type, min, max });
    executablePcBody = removeMatchedSlice(executablePcBody, match);
  }
  const invariantPattern = /^\s*invariant\s+([^;]+)\s*;\s*/m;
  while (true) {
    const match = executablePcBody.match(invariantPattern);
    if (!match) break;
    const expression = match[1].trim();
    if (!expression) fail(65, 'pcd_parse_error:empty_invariant');
    if (/[^A-Za-z0-9_+\-*/%<>=!&|()[\],{}:.\s]/.test(expression)) fail(65, 'pcd_parse_error:unsupported_invariant_expression');
    invariants.push({ expression });
    executablePcBody = removeMatchedSlice(executablePcBody, match);
  }
  const whenPattern = /^\s*when\s+([^{}]+)\s*\{\s*domain\s+([A-Za-z_][A-Za-z0-9_]*)\s*:\s*([A-Za-z0-9_]+)\s*\[\s*([^,\]]+)\s*,\s*([^\]]+)\s*\]\s*;\s*\}\s*/m;
  while (true) {
    ...
    validateDomainBoundForType(max, type, name, 'max');
    if (min.kind === 'literal' && max.kind === 'literal' && min.value > max.value) fail(65, `pcd_parse_error:domain_bounds_reversed:${name}`);
    conditionalDomains.push({
      condition: condition.trim(),
      domain: { name, type, min, max }
    });
    executablePcBody = removeMatchedSlice(executablePcBody, match);
  }
  return { executablePcBody, domains, domainParams, invariants, conditionalDomains };
}

function domainBoundValue(bound, technicalSheet = {}) {
  if (bound.kind === 'literal') return bound.value;
  if (bound.kind === 'parameter' && Object.prototype.hasOwnProperty.call(technicalSheet, bound.name)) {
    const value = Number(technicalSheet[bound.name]);
    if (!Number.isFinite(value)) fail(65, `domain_param_value_invalid:${bound.name}`);
    return value;
  }
  return null;
}

function buildDomainContract({ domains, domainParams, invariants, conditionalDomains }, entryParams = [], technicalSheet = {}) {
  const declared = new Set(domains.map((domain) => domain.name));
  const missingParams = entryParams.filter((param) => !declared.has(param));
  const referencedParams = new Set();
  for (const domain of domains) {
    if (domain.min.kind === 'parameter') referencedParams.add(domain.min.name);
    if (domain.max.kind === 'parameter') referencedParams.add(domain.max.name);
  }
  for (const conditional of conditionalDomains) {
    if (conditional.domain.min.kind === 'parameter') referencedParams.add(conditional.domain.min.name);
    if (conditional.domain.max.kind === 'parameter') referencedParams.add(conditional.domain.max.name);
  }
  const declaredDomainParams = new Set(domainParams.map((param) => param.name));
  for (const name of referencedParams) {
  ...
  const normalized = {
    ...
    claimBearingEligible: missingParams.length === 0 && unresolvedParams.length === 0
  };
  return {
    ...normalized,
...
function loadTechnicalSheet(file) {
  if (!file) return {};
  const resolved = workspacePath(file, 64, { mustExist: true, realpath: true });
  const parsed = readJsonRequired(resolved, 'technical_sheet_parse_error', `technical_sheet_required:${file}`);
  return parsed.domainParams || parsed.params || parsed;
}
...
function parsePcd(source, context = {}) {
  ...
  let executablePcBody = parsedDomainContract.executablePcBody;
  ...
  if (/\bdomain(?:_param)?\s+[A-Za-z_][A-Za-z0-9_]*\b/.test(executablePcBody) || /\binvariant\s+/.test(executablePcBody) || /\bwhen\s+/.test(executablePcBody)) {
    fail(65, 'pcd_parse_error:malformed_domain_contract');
  }
  ...
  const domainContract = buildDomainContract(parsedDomainContract, entry.params);
  return {
...
function templateCommand(args = []) {
  ...
  const templates = {
    ...
    'domain-gate': `// brik64.pcd_file.v1\n// claim_boundary: local_candidate_only\nPC bounded_gate {\n    domain amount: i64 [0, 100000];\n    domain balance: i64 [0, 100000];\n    invariant amount <= balance;\n    fn bounded_gate(amount: i64, balance: i64) -> i64 {\n        if (amount <= balance) return 1;\n        return 0;\n    }\n}\n`
  };
...
function pcdInventory(options = {}) {
  const results = [];
  const ignored = new Set(['.git', '.brik', 'node_modules', 'target', 'dist', 'build', 'evidence', 'engines', 'out-ts', 'out-rust', 'out-python']);
  const projectIgnored = new Set(['adversarial', 'attacks', 'fuzz', 'scratch', 'tmp']);
  function walk(dir) {
    if (!fs.existsSync(dir)) return;
    for (const name of fs.readdirSync(dir).sort()) {
      if (ignored.has(name)) continue;
      if ((options.scope || 'project') === 'project' && projectIgnored.has(name)) continue;
...
function buildDoctorReport(options = {}) {
  ...
  if (manifest) {
    ...
    const publicRuntime = policy.runtimeMode || policy.publicOfflineRuntime;
    const managedRuntime = policy.managedRuntime || policy.registeredManagedRuntime;
    const artifactFactory = policy.artifactFactory || policy.internalArtifactFactory;
    const publicDistribution = policy.publicDistribution ?? policy.l6DistributionAllowed;
    const embeddedManagedRuntime = policy.embeddedManagedRuntime ?? policy.l5EmbeddedFreeRuntimeAllowed;
    if (publicRuntime !== 'local_runtime') {
      errors.push('engine_tier_policy_missing_local_runtime');
      actions.push('Restore engineTierPolicy.runtimeMode to local_runtime.');
    }
    if (managedRuntime !== 'managed_platform') {
      errors.push('engine_tier_policy_missing_managed_platform');
      actions.push('Restore engineTierPolicy.managedRuntime to managed_platform.');
    }
    if (artifactFactory !== 'private_factory') {
      errors.push('engine_tier_policy_missing_private_factory');
      actions.push('Restore engineTierPolicy.artifactFactory to private_factory.');
    }
    if (publicDistribution !== false) {
      errors.push('engine_tier_policy_public_distribution_open');
      actions.push('Set engineTierPolicy.publicDistribution to false.');
    }
    if (embeddedManagedRuntime !== false) {
      errors.push('engine_tier_policy_embedded_managed_runtime_open');
      actions.push('Set engineTierPolicy.embeddedManagedRuntime to false.');
    }
  ...
  if (!['project', 'all'].includes(scope)) fail(64, `doctor_scope_unsupported:${scope}`);
  const pcds = pcdInventory({ scope });
  const pcdValidation = [];
  for (const item of pcds) {
    ...
    if (validation.ok) {
      pcdValidation.push({
        file: item.file,
        status: validation.domainContract.claimBearingEligible ? 'PASS' : 'WARN',
        domainStatus: validation.domainContract.status,
        domain_contract_sha256: validation.domainContract.sha256
      });
      if (!validation.domainContract.claimBearingEligible) {
        warnings.push(`pcd_domain_incomplete:${item.file}:${validation.domainContract.status}`);
        actions.push(`Add domain declarations to ${item.file}; run \`brik64 domain inspect ${item.file} --json\`.`);
      }
    } else {
  ...
  return {
    ...
    artifactFactory: 'private',
    scanScope: scope,
    pcdCount: pcds.length,
    pcdValidation: {
      valid: pcdValidation.filter((item) => item.status === 'PASS' || item.status === 'WARN').length,
      invalid: pcdValidation.filter((item) => item.status === 'FAIL').length,
      domainIncomplete: pcdValidation.filter((item) => item.status === 'WARN').length,
      files: pcdValidation
    },
...
function validatePcdFileWithExplain(file) {
  const result = spawnSync(process.execPath, [__filename, 'domain', 'inspect', file, '--json', '--quiet'], {
    cwd: process.cwd(),
    encoding: 'utf8',
  ...
  if (result.status === 0) {
    try {
      ...
      return { ok: true, domainContract: report.domainContract };
    } catch (_) {
      return { ok: false, error: 'domain_report_parse_error' };
    }
  }
  return {
    ok: false,
...
function doctor() {
  const args = parseArgs(process.argv.slice(3), { '--json': 'boolean', '--scope': 'value' });
  const report = buildDoctorReport({ scope: args['--scope'] || 'project' });
  if (args['--json']) {
...
function renderExpression(expression, target) {
  ...
  if (expression.type === 'MonomerCallExpression') {
    ...
    if (expression.operation === 'clamp') {
      ...
      if (target === 'rust') {
        const argTypes = expression.args.map((argument) => argument.inferredType || argument.numericType);
        const clampType = expression.inferredType === 'f64' || argTypes.includes('f64') ? 'f64' : 'i64';
        const rustArgs = expression.args.map((argument) => renderRustExpressionAs(argument, clampType));
        return `(${rustArgs[0]}).max(${rustArgs[1]}).min(${rustArgs[2]})`;
      }
  ...
  if (expression.type === 'BinaryExpression') {
    const rustF64Binary = target === 'rust' && (
      expression.left.inferredType === 'f64' ||
      expression.right.inferredType === 'f64' ||
      expression.inferredType === 'f64'
    );
    const left = rustF64Binary ? renderRustExpressionAs(expression.left, 'f64') : renderExpression(expression.left, target);
    const right = rustF64Binary ? renderRustExpressionAs(expression.right, 'f64') : renderExpression(expression.right, target);
...
function renderPcdDomainContract(contract, indentLevel = 1) {
  if (!contract) return [];
  const indent = '    '.repeat(indentLevel);
  const bound = (value) => value.kind === 'literal' ? domainLiteral(value.value) : value.name;
  const lines = [];
  for (const param of contract.domainParams || []) {
    lines.push(`${indent}domain_param ${param.name}: ${param.type};`);
  }
  for (const domain of contract.domains || []) {
    lines.push(`${indent}domain ${domain.name}: ${domain.type} [${bound(domain.min)}, ${bound(domain.max)}];`);
  }
  for (const invariant of contract.invariants || []) {
    lines.push(`${indent}invariant ${invariant.expression};`);
  }
  for (const conditional of contract.conditionalDomains || []) {
    const domain = conditional.domain;
    lines.push(`${indent}when ${conditional.condition} { domain ${domain.name}: ${domain.type} [${bound(domain.min)}, ${bound(domain.max)}]; }`);
  }
  return lines;
}
...
function generatedCases(ast) {
  ...
  const domainByName = Object.fromEntries((ast.domainContract?.domains || [])
    .filter((domain) => domain.min.kind === 'literal' && domain.max.kind === 'literal')
  ...
  return inputs
    .map((input) => {
      const args = Object.fromEntries(params.map((param, index) => {
        const domain = domainByName[param];
        if (!domain) return [param, index === 0 ? input : 1];
        if (index === 0) {
          const bounded = Math.max(domain.min.value, Math.min(domain.max.value, input));
          return [param, bounded];
        }
        return [param, domain.max.value];
      }));
      return {
        input,
        args,
...
function domainLiteral(value) {
  return Number.isInteger(value) ? String(value) : String(Number(value));
}

function rustDomainLiteral(value, type) {
  if (type === 'f64') return Number.isInteger(value) ? `${value}.0` : String(Number(value));
  return domainLiteral(value);
}

function renderDomainAssertions(ast, target) {
  const contract = ast.domainContract || { domains: [], invariants: [] };
  const domains = contract.domains.filter((domain) => domain.min.kind === 'literal' && domain.max.kind === 'literal');
  if (target === 'python') {
    const lines = [`DOMAIN_CONTRACT_SHA256 = "${contract.sha256 || ''}"`, '', 'def assertDomain(**kwargs):'];
    for (const param of ast.params.length > 0 ? ast.params : ['input']) {
      lines.push(`    ${param} = kwargs.get("${param}", 0)`);
    }
    if (domains.length === 0 && (contract.invariants || []).length === 0) lines.push('    return True');
    for (const domain of domains) {
      lines.push(`    if "${domain.name}" in kwargs and not (${domainLiteral(domain.min.value)} <= kwargs["${domain.name}"] <= ${domainLiteral(domain.max.value)}):`);
      lines.push(`        raise ValueError("domain_out_of_bounds:${domain.name}")`);
    }
    for (const invariant of contract.invariants || []) {
      lines.push(`    if not (${invariant.expression}):`);
      lines.push('        raise ValueError("domain_invariant_failed")');
    }
  ...
  const lines = [`export const domainContractSha256 = "${contract.sha256 || ''}";`, 'export function assertDomain(args = {}) {'];
  if ((ast.params || []).length > 0) {
    lines.push(`  const { ${ast.params.map((param) => `${param} = 0`).join(', ')} } = args;`);
  }
  if (domains.length === 0 && (contract.invariants || []).length === 0) lines.push('  return true;');
  for (const domain of domains) {
    lines.push(`  if (Object.prototype.hasOwnProperty.call(args, "${domain.name}") && !(${domainLiteral(domain.min.value)} <= args.${domain.name} && args.${domain.name} <= ${domainLiteral(domain.max.value)})) throw new Error("domain_out_of_bounds:${domain.name}");`);
  }
  for (const invariant of contract.invariants || []) {
    lines.push(`  if (!(${invariant.expression})) throw new Error("domain_invariant_failed");`);
  }
  lines.push('  return true;', '}', '');
  return lines;
}
...
function targetSpec(target, ast) {
  ...
  const rustProgram = (hash) => [
    ...
    ...rustDomainAssertions,
    ...rustImports,
    ...rustHelpers,
    `pub fn run(${rustParams}) -> ${rustType(ast.returnType)} {`,
    `    assert_domain(${params.join(', ')});`,
...
function domainCommand(args = []) {
  const sub = args[0];
  if (!sub) fail(64, 'domain_subcommand_required');
  if (sub === 'inspect' || sub === 'validate' || sub === 'sheet') {
    const file = args[1];
    const parsed = parseArgs(args.slice(2), { '--json': 'boolean', '--out': 'value', '--technical-sheet': 'value' });
    ...
    const technicalSheet = loadTechnicalSheet(parsed['--technical-sheet']);
    const contract = Object.keys(technicalSheet).length > 0
      ? buildDomainContract(ast.domainContract, ast.params, technicalSheet)
      : ast.domainContract;
    const report = {
      schemaVersion: 'brik64.cli_domain_report.v1',
      cliVersion: version,
      command: sub,
      pcd: file,
      domainContract: contract,
      claimBearingEligible: contract.claimBearingEligible,
      action: contract.claimBearingEligible
        ? 'Domain contract is complete for local candidate certification.'
        : 'Add domain declarations or provide a technical sheet for domain_param values.'
    };
    if (sub === 'sheet') {
      const sheet = {
        schemaVersion: 'brik64.cli_domain_technical_sheet.v1',
        cliVersion: version,
        pcd: file,
        domain_contract_sha256: ast.domainContract.sha256,
        domainParams: Object.fromEntries((ast.domainContract.domainParams || []).map((param) => [param.name, null])),
        domains: ast.domainContract.domains,
        invariants: ast.domainContract.invariants,
        conditionalDomains: ast.domainContract.conditionalDomains
      };
      const out = parsed['--out'];
      if (out) {
        const outPath = workspacePath(out, 64, { output: true });
        mkdirControlled(path.dirname(outPath));
        writeFileControlled(outPath, JSON.stringify(sheet, null, 2) + '\n');
        process.stdout.write(`domain_sheet=${path.relative(process.cwd(), outPath)}\n`);
        return;
      }
    ...
    if (parsed['--json']) {
      ...
      process.stdout.write(`domain_status=${contract.status}\n`);
      process.stdout.write(`domain_contract_sha256=${contract.sha256}\n`);
      process.stdout.write(`claim_bearing_eligible=${contract.claimBearingEligible ? 'yes' : 'no'}\n`);
      if (contract.missingParams.length > 0) process.stdout.write(`missing_params=${contract.missingParams.join(',')}\n`);
      if (contract.unresolvedParams.length > 0) process.stdout.write(`unresolved_params=${contract.unresolvedParams.join(',')}\n`);
    }
    if (sub === 'validate' && !contract.claimBearingEligible) process.exit(65);
    return;
  }
  if (sub === 'add') {
    const file = args[1];
    ...
    if (!parsed['--param'] || !parsed['--min'] || !parsed['--max']) fail(64, 'domain_add_requires_param_min_max');
    const resolvedFile = workspacePath(file, 64, { mustExist: true, realpath: true });
    const source = readFileRequired(file);
    const ast = parsePcd(source, { baseDir: path.dirname(resolvedFile), importStack: [resolvedFile] });
    if (ast.domainContract.domains.some((domain) => domain.name === parsed['--param']) && !parsed['--force']) {
      fail(73, `domain_exists:${parsed['--param']}`);
    }
    const declaration = `    domain ${parsed['--param']}: ${parsed['--type'] || 'i64'} [${parsed['--min']}, ${parsed['--max']}];\n`;
    const pcStart = source.search(/\bPC\s+[A-Za-z_][A-Za-z0-9_]*\s*\{/m);
    if (pcStart === -1) fail(65, 'pcd_parse_error:missing_pc_block');
    const pcOpen = source.indexOf('{', pcStart);
    const nextSource = `${source.slice(0, pcOpen + 1)}\n${declaration}${source.slice(pcOpen + 1)}`;
    parsePcd(nextSource, { baseDir: path.dirname(resolvedFile), importStack: [resolvedFile] });
    writeFileControlled(resolvedFile, nextSource);
    process.stdout.write(`domain_added=${parsed['--param']}\n`);
    return;
  }
  fail(64, `unknown_domain_command:${sub}`);
}

function requireClaimReadyDomain(ast, options = {}) {
  if (ast.domainContract.claimBearingEligible) return;
  if (options.prototypeNonClaim || options.syntaxOnly) return;
  const status = ast.domainContract.status;
  const missing = ast.domainContract.missingParams.join(',');
  const unresolved = ast.domainContract.unresolvedParams.join(',');
  fail(65, `bounded_domain_required:${status}; missing=${missing || 'none'}; unresolved=${unresolved || 'none'}; run \`brik64 domain inspect ${options.file || '<file.pcd>'} --json\` or add domain declarations`);
}

function certify(file, args = []) {
  validateManifest();
  const parsed = parseArgs(args, { '--syntax-only': 'boolean', '--prototype-non-claim': 'boolean' });
  const resolvedFile = workspacePath(file, 64, { mustExist: true, realpath: true });
  const source = readFileRequired(file);
  const ast = parsePcd(source, { baseDir: path.dirname(resolvedFile), importStack: [resolvedFile] });
  ...
  const cert = {
    ...
    domain_contract_sha256: ast.domainContract.sha256,
    domainContract: ast.domainContract,
    ast,
...
function certificateFor(file, source, ast) {
  ...
  if (cert.domain_contract_sha256 && cert.domain_contract_sha256 !== ast.domainContract.sha256) {
    fail(68, 'certificate_domain_contract_mismatch');
  }
  return cert;
}
...
function verify(file, args = []) {
  ...
  const report = {
    ...
    semantic_pcd_sha256: cert.semantic_pcd_sha256,
    ast_sha256: cert.ast_sha256,
    domain_contract_sha256: cert.domain_contract_sha256 || ast.domainContract.sha256,
    domainStatus: ast.domainContract.status,
    checks: {
      parseable: true,
      certificatePresent: true,
      certificateHashMatches: true,
      astHashMatches: true,
      domainHashMatches: (cert.domain_contract_sha256 || ast.domainContract.sha256) === ast.domainContract.sha256
    },
    claimBoundary: {
      localCandidateOnly: true,
      universalCorrectnessClaimAllowed: false,
...
function polymerize(rawArgs = []) {
  ...
  const units = files.map((file) => {
    ...
    const source = readFileRequired(file);
    const ast = parsePcd(source, { baseDir: path.dirname(resolvedFile), importStack: [resolvedFile] });
    requireClaimReadyDomain(ast, { file });
    return {
      file,
      resolvedFile,
      semantic_pcd_sha256: sha256(source),
  ...
  const manifestSources = units.map((unit) => ({
    ...
    domainContract: unit.ast.domainContract,
    ast: unit.ast
  }));
  const manifest = {
...
function migrate(file, args = []) {
  ...
  if (syntax !== 'beta14' && !/\bdomain\s+[A-Za-z_][A-Za-z0-9_]*\s*:/.test(migrated)) {
    ...
    const domainLines = params.map((param) => `    domain ${param}: i64 [0, 255];`).join('\n');
    if (domainLines) {
...
function rewriteClampExpression(value) {
  const source = String(value || '').trim();
  const patterns = [
    {
      re: /^Math\.floor\s*\(\s*Math\.min\s*\(\s*Math\.max\s*\((.+?),\s*(.+?)\)\s*,\s*(.+?)\s*\)\s*\)$/,
      render: (match) => `MC_79.FLOOR(MC_07.CLAMP(${match[1].trim()}, ${match[2].trim()}, ${match[3].trim()}))`
    },
    {
      re: /^floor\s*\(\s*min\s*\(\s*max\s*\((.+?),\s*(.+?)\)\s*,\s*(.+?)\s*\)\s*\)$/,
      render: (match) => `MC_79.FLOOR(MC_07.CLAMP(${match[1].trim()}, ${match[2].trim()}, ${match[3].trim()}))`
    },
    {
      re: /^Math\.min\s*\(\s*Math\.max\s*\((.+?),\s*(.+?)\)\s*,\s*(.+?)\s*\)$/,
      render: (match) => `MC_07.CLAMP(${match[1].trim()}, ${match[2].trim()}, ${match[3].trim()})`
    },
    {
      re: /^min\s*\(\s*max\s*\((.+?),\s*(.+?)\)\s*,\s*(.+?)\s*\)$/,
      render: (match) => `MC_07.CLAMP(${match[1].trim()}, ${match[2].trim()}, ${match[3].trim()})`
    }
  ];
...
function normalizeLiftExpression(expression, language) {
  ...
  value = value
    ...
    .replace(/\bMath\.floor\s*\(/g, 'MC_79.FLOOR(')
...
function simpleBodyLinesFromSource(body, language) {
  ...
  const blockIfPattern = language === 'python'
    ? /^[ \t]*if\s+(.+?)\s*:\s*\n[ \t]+return\s+(.+?)(?:\n[ \t]*else\s*:\s*\n[ \t]+return\s+(.+?))?$/gm
    : language === 'rust'
  ...
  while ((match = blockIfPattern.exec(body))) {
    ...
    if (condition && consequent && !lines.includes(`        if (${condition}) return ${consequent};`)) {
      lines.push(`        if (${condition}) return ${consequent};`);
    }
    if (alternate && !lines.includes(`        return ${alternate};`)) {
...
function buildCandidatePcd(name, params, expression, options = {}) {
  ...
  return [
    ...
    ...sourceComment,
    'PC ' + safeName + ' {',
    ...safeParams.map((param) => `    domain ${param}: i64 [-1000000, 1000000];`),
...
function extractRustLiftCandidates(source) {
  const candidates = [];
  const warnings = [];
  const functionPattern = /\b(?:pub\s+)?fn\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)\s*(?:->\s*[A-Za-z0-9_]+)?\s*\{/g;
  let match;
  while ((match = functionPattern.exec(source))) {
    const name = match[1];
    const params = match[2].split(',').map((param) => param.trim().replace(/:.*/, '').trim()).filter(Boolean);
    ...
    if (depth !== 0) {
      warnings.push({ code: 'unsupported_construct', function: name, reason: 'unbalanced_rust_function_body' });
      continue;
    }
    const body = source.slice(bodyStart, cursor - 1);
    functionPattern.lastIndex = cursor;
    const returnMatch = body.match(/\breturn\s+([^;]+);?/) || body.trim().match(/^(.+?);?$/m);
...
async function main() {
  ...
  if (cmd === 'certify') return certify(file, args);
