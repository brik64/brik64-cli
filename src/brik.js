...
function renderSemanticPolymer(rootUnit, units) {
  ...
  return [
    '// brik64.pcd_file.v1',
    '// generated_by: brik64-cli beta12 semantic polymerize local',
...
function targetSpec(target, ast) {
  ...
  const tsProgram = (hash) => [
    '// BRIK64 beta12 functional emission candidate',
...
function emit(file, args = []) {
  ...
  process.stdout.write('// BRIK64 beta12 functional emission candidate\n');
...
function verify(file, args = []) {
  ...
  if (parsed['--cloud']) {
    fail(69, 'managed_verify_endpoint_unavailable_beta12');
  }
  const resolvedFile = workspacePath(file, 64, { mustExist: true, realpath: true });
  const source = readFileRequired(file);
  const ast = parsePcd(source, { baseDir: path.dirname(resolvedFile), importStack: [resolvedFile] });
...
function polymerize(rawArgs = []) {
  ...
  requireLocalOrEntitled(parsed);
  if (parsed['--cloud']) {
    fail(69, 'managed_polymerize_endpoint_unavailable_beta12');
  }
...
function migrate(file, args = []) {
  ...
  if (parsed['--in-place']) {
    ...
    outPath = workspacePath(parsed['--out'] || `${file.replace(/\.pcd$/, '')}.beta12.pcd`, 64, { output: true });
...
async function telemetry(args = []) {
  ...
  if (sub === 'explain') {
    process.stdout.write('BRIK64 telemetry is disabled by default in beta12.\n');
    ...
    process.stdout.write('beta12 networkSent=false unless telemetry send is explicitly invoked after enable\n');
    return;
  }
