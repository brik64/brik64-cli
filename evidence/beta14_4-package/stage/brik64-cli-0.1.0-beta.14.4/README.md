...
Current public beta: `0.1.0-beta.14.4`
...
## Beta14.4 Command Surface

`0.1.0-beta.14.4` extends the Beta14 public CLI with a 128-monomer inspection surface, executable or boundary-contract fixtures for every registry entry, Rust emission fixes, Rust lift preview, directory lift handling, and the same local-candidate claim boundary.
...
- `brik64 lift <js|ts|python|rust> <path> --preview` scans simple local source
  ...
  `.brik/lift-preview/`. Beta14.4 translates simple `if`/`return` bodies,
  arithmetic, `Math.*`/`abs` helpers, clamp-style `min(max())` patterns, and
  simple Rust `fn`/`pub fn` returns. Unsupported constructs produce an
...
```sh
npm install @brik64/core@0.1.0-beta.14.4
pip install brik64==0.1.0b14.post4
cargo add brik64-core@0.1.0-beta.14.4
```
