...
function targetSpec(target, ast) {
  ...
  const tsProgram = (hash) => [
    '// BRIK64 beta11 functional emission candidate',
...
function emit(file, args = []) {
  ...
  process.stdout.write('// BRIK64 beta11 functional emission candidate\n');
...
function verify(file, args = []) {
  ...
  if (parsed['--cloud']) {
    fail(69, 'managed_verify_endpoint_unavailable_beta11');
  }
  const resolvedFile = workspacePath(file);
  const source = readFileRequired(file);
  const ast = parsePcd(source, { baseDir: path.dirname(resolvedFile), importStack: [resolvedFile] });
...
function polymerize(rawArgs = []) {
  ...
  requireLocalOrEntitled(parsed);
  if (parsed['--cloud']) {
    fail(69, 'managed_polymerize_endpoint_unavailable_beta11');
  }
...
function migrate(file, args = []) {
  ...
  if (parsed['--in-place']) {
    ...
    outPath = workspacePath(parsed['--out'] || `${file.replace(/\.pcd$/, '')}.beta11.pcd`);
...
function telemetry(args = []) {
  ...
  if (sub === 'explain') {
    process.stdout.write('BRIK64 telemetry is disabled by default in beta11.\n');
    process.stdout.write('When enabled in a future transport release it must be opt-in, redacted, exportable, and purgeable.\n');
    process.stdout.write('beta11 networkSent=false\n');
    return;
  }
...
function feedback(args = []) {
  ...
  if (!parsed['--dry-run']) fail(64, 'feedback_requires_dry_run_in_beta11');
