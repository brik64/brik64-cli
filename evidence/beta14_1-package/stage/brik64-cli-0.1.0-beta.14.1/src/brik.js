...
const COMMAND_HELP = {
  ...
  lift: [
    ...
    'Creates local PCD candidates. By default Beta14.1 translates simple if/return patterns.',
    'Example:',
  ...
  template: [
    ...
    'Writes a starter PCD matching the public Beta14.1 parser profile.',
    'Example:',
...
function printCommandHelp(name) {
  ...
  if (name !== 'exit-codes') {
    ...
    process.stdout.write('  Numeric monomers supported: MC_00.ADD8, MC_01.SUB8, MC_02.MUL8, MC_03.DIV8, MC_04.MOD8.\n');
  }
}
...
function parseExpression(source, params, imports = {}, constants = {}, localFunctions = {}) {
  ...
  function parsePrimary() {
    ...
    if (token.type === 'number') {
      ...
      consume();
      if (/^MC_\d{2}$/.test(token.value) && peek() && peek().value === '.') {
        ...
        const spec = supportedNumericMonomer(`${monomerId}.${monomerName}`);
        if (!spec) {
          fail(65, `pcd_parse_error:unsupported_monomer_call:${monomerId}.${monomerName}; public beta14.1 supports numeric PCD monomers only. Use SDK mc.* for string/extended helpers.`);
        }
        consume('(');
        const args = [];
        if (peek() && peek().value !== ')') {
        ...
        expression = { type: 'MonomerCallExpression', monomer: spec.id, operation: spec.operation, args };
...
function supportedNumericMonomer(id) {
  const spec = {
    'MC_00.ADD8': { operation: 'add', arity: 2 },
    'MC_01.SUB8': { operation: 'sub', arity: 2 },
    'MC_02.MUL8': { operation: 'mul', arity: 2 },
    'MC_03.DIV8': { operation: 'div', arity: 2 },
    'MC_04.MOD8': { operation: 'mod', arity: 2 }
  }[id];
...
function renderExpression(expression, target) {
  ...
  if (expression.type === 'MonomerCallExpression') {
    ...
    if (expression.operation === 'add') return `(${args[0]} + ${args[1]})`;
    if (expression.operation === 'sub') return `(${args[0]} - ${args[1]})`;
    if (expression.operation === 'mul') return `(${args[0]} * ${args[1]})`;
    if (expression.operation === 'div') {
      if (target === 'python') return `int(${args[0]} / ${args[1]})`;
      if (target === 'rust') return `(${args[0]} / ${args[1]})`;
      return `Math.trunc(${args[0]} / ${args[1]})`;
    }
    if (expression.operation === 'mod') return `(${args[0]} % ${args[1]})`;
...
function renderInlinePolymer(units) {
  ...
  const lines = [
    '// brik64.pcd_file.v1',
    '// generated_by: brik64-cli beta14.1 semantic polymerize local',
    '// claim_boundary: local_candidate_only',
...
function evaluateExpression(expression, env) {
  ...
  if (expression.type === 'MonomerCallExpression') {
    ...
    if (expression.operation === 'sub') return args[0] - args[1];
    if (expression.operation === 'mul') return args[0] * args[1];
    if (expression.operation === 'div') return args[1] === 0 ? undefined : Math.trunc(args[0] / args[1]);
    if (expression.operation === 'mod') return args[1] === 0 ? undefined : args[0] % args[1];
    return undefined;
  }
  if (expression.type === 'BinaryExpression') {
...
function polymerize(rawArgs = []) {
  ...
  const content = parsed['--inline'] ? renderInlinePolymer(units) : renderSemanticPolymer(rootUnit, units);
  writeFileControlled(outPath, content);
  const materializedImports = parsed['--inline'] ? [] : materializePolymerImports(rootUnit, outPath);
  ...
  const manifest = {
    ...
    cliVersion: version,
    mode: 'local',
    semanticMode: parsed['--inline'] ? 'inline_merged_functions' : 'root_dag_reference',
    root: {
      file: rootUnit.file,
...
function buildCandidatePcd(name, params, expression, options = {}) {
  ...
  return [
    '// brik64.pcd_file.v1',
    '// generated_by: brik64-cli beta14.1 lift preview',
    '// claim_boundary: local_candidate_only',
    ...sourceComment,
    'PC ' + safeName + ' {',
