...
function renderLocalFunctions(ast, target) {
  ...
  for (const fn of Object.values(ast.functions || {}).filter((candidate) => candidate.name !== ast.fnName)) {
    ...
    if (target === 'rust') {
      lines.push(`fn ${fn.name}(${params.map((param) => `${param}: i64`).join(', ')}) -> i64 {`);
...
function renderSemanticPolymer(rootUnit, units) {
  ...
  return [
    '// brik64.pcd_file.v1',
    '// generated_by: brik64-cli beta13 semantic polymerize local',
...
function targetSpec(target, ast) {
  ...
  const tsProgram = (hash) => [
    '// BRIK64 beta13 functional emission candidate',
...
function emit(file, args = []) {
  ...
  if (options.target || options.outDir || options.tests) {
    ...
    const outDir = workspacePath(options.outDir, 64, { output: true });
  ...
  process.stdout.write('// BRIK64 beta13 functional emission candidate\n');
...
function verify(file, args = []) {
  ...
  if (parsed['--cloud']) {
    fail(69, 'managed_verify_endpoint_unavailable_beta13');
  }
  const resolvedFile = workspacePath(file, 64, { mustExist: true, realpath: true });
  const source = readFileRequired(file);
  const ast = parsePcd(source, { baseDir: path.dirname(resolvedFile), importStack: [resolvedFile] });
...
function polymerize(rawArgs = []) {
  ...
  requireLocalOrEntitled(parsed);
  if (parsed['--cloud']) {
    fail(69, 'managed_polymerize_endpoint_unavailable_beta13');
  }
...
function migrate(file, args = []) {
  ...
  if (parsed['--in-place']) {
    ...
    outPath = workspacePath(parsed['--out'] || `${file.replace(/\.pcd$/, '')}.beta13.pcd`, 64, { output: true });
...
function buildCandidatePcd(name, params, expression) {
  ...
  return [
    '// brik64.pcd_file.v1',
    '// generated_by: brik64-cli beta13 lift preview',
    '// claim_boundary: local_candidate_only',
    'PC ' + safeName + ' {',
...
async function telemetry(args = []) {
  ...
  if (sub === 'explain') {
    process.stdout.write('BRIK64 telemetry is disabled by default in beta13.\n');
    ...
    process.stdout.write('beta13 networkSent=false unless telemetry send is explicitly invoked after enable\n');
    return;
  }
