...
OUT_DIR="$ROOT_DIR/evidence/beta15_1-package"
...
ledger_gate="$ROOT_DIR/evidence/beta15_1-ledger-dtl/report.json"
ledger_gate_decision="$(test -f "$ledger_gate" && json_field "$ledger_gate" '.decision' || true)"
...
[[ "$ledger_gate_decision" == "PASS_LOCAL_CANDIDATE_GATE" ]] || failures+=("beta15_1_ledger_gate_missing_or_invalid:$ledger_gate_decision")
...
if [[ "${#failures[@]}" -gt 0 ]]; then
  ...
  printf '%s\n' "${failures[@]}" | jq -Rsc \
    --arg version "$VERSION" \
    '{
      schemaVersion:"brik64.cli_beta15_1_package_manifest.v1",
      version:$version,
      decision:"FAIL_BRIK64_CLI_BETA15_1_PACKAGE_BUILT",
      releaseEligible:false,
      failures:(split("\n") | map(select(length > 0)))
    }' > "$MANIFEST_PATH"
  printf 'beta15_1_package_input_gate_failed:%s\n' "${failures[*]}" >&2
  exit 1
fi
...
inputs=(
  ...
  "pcd/beta15"
  "pcd/beta15_1"
  "docs/BETA15_1_LEDGER_DTL_IMPLEMENTATION_STATUS.md"
  "evidence/beta15_1-ledger-dtl/report.json"
...
jq -n \
  --arg version "$VERSION" \
  --arg packagePath "evidence/beta15_1-package/$PACKAGE_NAME" \
  ...
  '{
    ...
    inputGates:[{
      decision:"PASS_LOCAL_CANDIDATE_GATE",
      report:"evidence/beta15_1-ledger-dtl/report.json",
      packaged:true
    }],
    requiredPublicReleaseGates:[
      "beta15_1_package_smoke",
      "github_release_beta15_1",
      "curl_gcp_installer_beta15_1",
      "web_docs_changelog_beta15_1",
      "skills_beta15_1",
      "sdk_beta15_1_marketplaces_or_deferrals",
      "public_claim_scan",
      "live_release_train_verify"
    ],
    boundary:"Beta15.1 local package candidate only. Public release remains blocked until package smoke, GitHub, curl/GCP, docs, web, changelog, SDK, skills, marketplace publication or deferral, and live verification pass atomically."
  }' > "$MANIFEST_PATH"
...
printf 'decision=PASS_BRIK64_CLI_BETA15_1_PACKAGE_BUILT\n'
printf 'releaseEligible=false\n'
printf 'package=evidence/beta15_1-package/%s\n' "$PACKAGE_NAME"
printf 'sha256=%s\n' "$package_sha"
