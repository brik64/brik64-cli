...
const evidenceDir = path.join(root, 'evidence', 'beta15_4-pre-public-rc');
const records = [];
...
try {
  ...
  run('node_check_beta15_4_rust_polymer_domain_gate', process.execPath, ['--check', 'scripts/beta15_4-rust-polymer-domain-gate.js']);
  run('node_check_beta15_4_stale_public_surface_gate', process.execPath, ['--check', 'scripts/beta15_4-stale-public-surface-gate.js']);
  run('node_check_self', process.execPath, ['--check', 'scripts/beta15_4-pre-public-rc-gate.js']);
  run('gate_beta15_4_rust_polymer_domain', process.execPath, ['scripts/beta15_4-rust-polymer-domain-gate.js'], {
    expect: 'PASS_BRIK64_CLI_BETA15_4_RUST_POLYMER_DOMAIN_GATE'
  });
  run('gate_beta15_4_stale_public_surface', process.execPath, ['scripts/beta15_4-stale-public-surface-gate.js'], {
    expect: 'PASS_BRIK64_CLI_BETA15_4_STALE_PUBLIC_SURFACE_GATE'
  });
  ...
  const rustPolymerGate = readJson('evidence/beta15_4-rust-polymer-domain/report.json');
  assert(rustPolymerGate.decision === 'PASS_BRIK64_CLI_BETA15_4_RUST_POLYMER_DOMAIN_GATE', 'rust_polymer_gate_not_pass', rustPolymerGate);

  const report = {
    schemaVersion: 'brik64.cli_beta15_4_pre_public_rc_report.v1',
    version,
    ...
    checked: [
      'node_syntax',
      'beta15_4_rust_polymer_domain',
      'beta15_4_stale_public_surface',
  ...
  process.stdout.write('decision=PASS_BRIK64_CLI_BETA15_4_PRE_PUBLIC_RC_GATE\n');
} catch (error) {
  const report = {
    schemaVersion: 'brik64.cli_beta15_4_pre_public_rc_report.v1',
    version,
    decision: 'FAIL_BRIK64_CLI_BETA15_4_PRE_PUBLIC_RC_GATE',
    evidenceLevel: 'NIVEL 3',
    releaseEligible: false,
    error: error.message,
