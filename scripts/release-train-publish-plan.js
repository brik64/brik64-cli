...
function main() {
  ...
  const commands = [
    ...
    command(
      ...
      `gh api repos/brik64/brik64-cli/git/ref/tags/${releaseTag} >/dev/null 2>&1 || gh api repos/brik64/brik64-cli/git/refs -f ref=refs/tags/${releaseTag} -f sha=$(git rev-parse HEAD); gh api --method PATCH repos/brik64/brik64-cli/git/refs/tags/${releaseTag} -f sha=$(git rev-parse HEAD) -F force=true; gh release view ${releaseTag} --repo brik64/brik64-cli || gh release create ${releaseTag} --repo brik64/brik64-cli --title "BRIK64 CLI ${manifest.version}" --notes-file CHANGELOG.md; gh release upload ${releaseTag} --repo brik64/brik64-cli --clobber ${packagePath} ${packageDir}/package.manifest.json ${packageDir}/SHA256SUMS; gh release edit ${releaseTag} --repo brik64/brik64-cli --title "BRIK64 CLI ${manifest.version}" --notes-file CHANGELOG.md --prerelease --draft=false`,
      true
    ),
    command(
      'sdk_npm',
...
PY`,
    ...
    command(
      ...
      `cargo info brik64-core@${manifest.version} >/dev/null 2>&1 || CARGO_REGISTRY_TOKEN="$BRIK64_CRATES_TOKEN" cargo publish --allow-dirty --manifest-path /Users/carlosjperez/Documents/GitHub/brik64-lib-rust/Cargo.toml`,
      true
    ),
    command(
      'gcp_curl',
