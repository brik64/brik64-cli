...
PKG_DIR="$ROOT_DIR/evidence/beta14_4-package"
OUT_DIR="$ROOT_DIR/evidence/beta14_4-package-smoke"
...
[[ "$manifest_decision" == "PASS_BRIK64_CLI_BETA14_4_PACKAGE_BUILT" ]] || { echo "package_decision_drift:$manifest_decision" >&2; exit 1; }
...
if [[ ! -e "$EXTRACTED/evidence" ]]; then
  echo "package_missing_beta14_4_evidence_payload" >&2
  exit 1
fi
...
jq -e '.decision == "PASS_BETA14_4_128_EXECUTABLE_GATE"' "$EXTRACTED/evidence/beta14_4-128-executable/report.json" >/dev/null || {
  echo "package_beta14_4_128_executable_gate_not_pass" >&2
  exit 1
}
...
jq -n \
  ...
  '{
    schemaVersion:"brik64.cli_beta14_4_package_smoke.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA14_4_LOCAL_PACKAGE_SMOKE",
...
printf 'decision=PASS_BRIK64_CLI_BETA14_4_LOCAL_PACKAGE_SMOKE\n'
printf 'checks=30\n'
