...
const outputRefs = {
  generatedArtifact: 'evidence/beta15_6-l6-generation/generated/brik64-cli.mjs',
  package: 'evidence/beta15_6-package/brik64-cli-0.1.0-beta.15.6.tgz',
  releaseManifest: 'release/manifest.json',
  sealReport: 'evidence/beta15_6-l6-generation/seal_report.json',
};
...
function buildRequest() {
  ...
  for (const [name, refPath] of Object.entries(outputRefs)) {
    if (name === 'generatedArtifact' || name === 'sealReport') continue;
    const absolutePath = path.join(root, refPath);
    if (!fs.existsSync(absolutePath) || !fs.statSync(absolutePath).isFile()) {
      throw new Error(`missing_output_artifact:${name}:${refPath}`);
    }
  ...
  return {
    ...
    version,
    lane: 'cli_0_1_beta',
    iterId: 'beta15.6-l6-cli-materializer-request',
...
function validateRequest(request) {
  ...
  for (const [name, artifact] of Object.entries(request?.outputArtifacts || {})) {
    if (!safeRelativePath(artifact?.path)) blockers.push(`request_output_artifact_path_invalid:${name}`);
    if (!/^[a-f0-9]{64}$/i.test(artifact?.sha256 || '')) blockers.push(`request_output_artifact_sha256_invalid:${name}`);
    if (!Number.isInteger(artifact?.bytes) || artifact.bytes < 1) blockers.push(`request_output_artifact_bytes_invalid:${name}`);
  }
  return {
...
function verifyBundle() {
  ...
  const validation = validateRequest(request);
  console.log(validation.accepted ? 'PASS beta15.6 L6 materializer request bundle' : 'FAIL beta15.6 L6 materializer request bundle');
  if (!validation.accepted) {
