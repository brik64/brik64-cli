...
const outDir = path.join(root, 'evidence', 'beta7-marketplace-packages');
const repos = {
...
function main() {
  ...
  const jsTarball = artifact(path.join(repos.js, 'evidence-beta7-pack', 'brik64-core-0.1.0-beta.7.tgz'), repos.js);
  const pythonWheel = artifact(path.join(repos.python, 'dist', 'brik64-0.1.0b7-py3-none-any.whl'), repos.python);
  const pythonSdist = artifact(path.join(repos.python, 'dist', 'brik64-0.1.0b7.tar.gz'), repos.python);
  const rustCrate = artifact(path.join(repos.rust, 'target', 'package', 'brik64-core-0.1.0-beta.7.crate'), repos.rust);
  ...
  if (jsVersion !== '0.1.0-beta.7') failures.push(`js_version_drift:${jsVersion || 'missing'}`);
  if (!/version\s*=\s*"0\.1\.0b7"/.test(pyProjectText)) failures.push('python_version_drift');
  if (!/version\s*=\s*"0\.1\.0-beta\.7"/.test(rustCargoText)) failures.push('rust_version_drift');
  if (!jsTarball) failures.push('npm_package_not_packed_for_beta7_publication');
  if (!pythonWheel || !pythonSdist) failures.push('pypi_package_not_built_for_beta7_publication');
  if (!rustCrate) failures.push('cargo_package_not_packed_for_beta7_publication');

  const report = {
    schemaVersion: 'brik64.cli_beta7_marketplace_package_gate.v1',
    ...
    failures,
    boundary: 'Package artifacts are locally built beta7 publication inputs. This gate verifies readiness but does not mutate npm, PyPI or Cargo.'
  };
