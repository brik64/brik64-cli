...
const outDir = path.join(root, 'evidence', 'beta5-marketplace-packages');
const repos = {
...
function main() {
  ...
  const jsTarball = artifact(path.join(repos.js, 'evidence-beta5-pack', 'brik64-core-0.1.0-beta.5.tgz'), repos.js);
  const pythonWheel = artifact(path.join(repos.python, 'dist', 'brik64-0.1.0b5-py3-none-any.whl'), repos.python);
  const pythonSdist = artifact(path.join(repos.python, 'dist', 'brik64-0.1.0b5.tar.gz'), repos.python);
  const rustCrate = artifact(path.join(repos.rust, 'target', 'package', 'brik64-core-0.1.0-beta.5.crate'), repos.rust);
  ...
  if (jsVersion !== '0.1.0-beta.5') failures.push(`js_version_drift:${jsVersion || 'missing'}`);
  if (!/version\s*=\s*"0\.1\.0b5"/.test(pyProjectText)) failures.push('python_version_drift');
  if (!/version\s*=\s*"0\.1\.0-beta\.5"/.test(rustCargoText)) failures.push('rust_version_drift');

  if (!jsTarball) failures.push('npm_package_not_packed_for_beta5_publication');
  if (!pythonWheel || !pythonSdist) failures.push('pypi_package_not_built_for_beta5_publication');
  if (!rustCrate) failures.push('cargo_package_not_packed_for_beta5_publication');

  const report = {
    schemaVersion: 'brik64.cli_beta5_marketplace_package_gate.v1',
    ...
    failures,
    boundary: 'Package artifacts are local beta5 candidates only. This gate does not authorize npm, PyPI or Cargo publication.'
  };
