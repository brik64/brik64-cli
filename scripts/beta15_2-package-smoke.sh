...
PKG_DIR="$ROOT_DIR/evidence/beta15_2-package"
OUT_DIR="$ROOT_DIR/evidence/beta15_2-package-smoke"
...
[[ "$manifest_decision" == "PASS_BRIK64_CLI_BETA15_2_PACKAGE_BUILT" ]] || { echo "package_decision_drift:$manifest_decision" >&2; exit 1; }
[[ "$release_eligible" == "false" ]] || { echo "beta15_2_candidate_should_not_be_public_release_eligible" >&2; exit 1; }
...
[[ -f "$EXTRACTED/evidence/beta15_2-pre-public-rc/report.json" ]] || {
  echo "package_missing_beta15_2_pre_public_evidence" >&2
  exit 1
}
jq -e '.decision == "PASS_BRIK64_CLI_BETA15_2_PRE_PUBLIC_RC_GATE"' "$EXTRACTED/evidence/beta15_2-pre-public-rc/report.json" >/dev/null || {
  echo "package_beta15_2_pre_public_gate_not_pass" >&2
  exit 1
}
[[ -f "$EXTRACTED/pcd/beta15_2/cli/domain_helper_enforcement.pcd" ]] || {
  echo "package_missing_beta15_2_pcd_contracts" >&2
  exit 1
}
...
jq -n \
  --arg version "$VERSION" \
  '{
    schemaVersion:"brik64.cli_beta15_2_package_smoke_report.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA15_2_PACKAGE_SMOKE",
    releaseEligible:false,
    checked:[
      "package_sha",
      "embedded_beta15_2_evidence",
      "version",
      "ledger_help",
      "certify",
...
printf 'decision=PASS_BRIK64_CLI_BETA15_2_PACKAGE_SMOKE\n'
