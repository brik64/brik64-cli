...
const evidenceDir = path.join(root, 'evidence', 'beta15_7-full-release-audit');
...
function runDirectFunctionalChecks() {
  ...
  assert(/^0\.1\.0-beta\.15\.7(?:\.\d+)?$/.test(version), 'version_not_beta15_7_family', { version });
...
try {
  ...
  const report = {
    schemaVersion: 'brik64.cli_beta15_7_full_release_audit_gate.v1',
    version,
    decision: 'PASS_BRIK64_CLI_BETA15_7_FULL_RELEASE_AUDIT_GATE',
  ...
  process.stdout.write('decision=PASS_BRIK64_CLI_BETA15_7_FULL_RELEASE_AUDIT_GATE\n');
} catch (error) {
  const report = {
    schemaVersion: 'brik64.cli_beta15_7_full_release_audit_gate.v1',
    version,
    decision: 'FAIL_BRIK64_CLI_BETA15_7_FULL_RELEASE_AUDIT_GATE',
    evidenceLevel: 'NIVEL 3',
    releaseEligible: false,
    publicationAllowed: false,
    workdir: work,
