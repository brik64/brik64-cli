...
try {
  ...
  const report = {
    schemaVersion: 'brik64.cli_beta15_4_stale_public_surface_gate.v1',
    version,
  ...
  const outDir = path.join(root, 'evidence', 'beta15_4-stale-public-surface');
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, 'report.json'), `${JSON.stringify(report, null, 2)}\n`);
  process.stdout.write('decision=PASS_BRIK64_CLI_BETA15_4_STALE_PUBLIC_SURFACE_GATE\n');
} catch (error) {
  const outDir = path.join(root, 'evidence', 'beta15_4-stale-public-surface');
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, 'report.json'), `${JSON.stringify({
    schemaVersion: 'brik64.cli_beta15_4_stale_public_surface_gate.v1',
    version,
    decision: 'FAIL_BRIK64_CLI_BETA15_4_STALE_PUBLIC_SURFACE_GATE',
    evidenceLevel: 'NIVEL 3',
    error: error.message,
    records: records.map((record) => ({
      id: record.id,
