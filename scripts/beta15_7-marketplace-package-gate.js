...
const outDir = path.join(root, 'evidence', 'beta15_7-marketplace-packages');
const version = '0.1.0-beta.15.7';
const pyVersion = '0.1.0b15.post7';
...
async function main() {
  ...
  if (npm.rc !== 0 || npm.stdout.trim() !== version) failures.push('npm_beta15_7_missing');

  try {
    const pypi = await fetchJson('https://pypi.org/pypi/brik64/json');
    if (!pypi.releases || !pypi.releases[pyVersion]) failures.push('pypi_beta15_7_missing');
  } catch (error) {
    failures.push(`pypi_lookup_failed:${error.message}`);
  }

  const crates = run('cargo', ['info', `brik64-core@${version}`]);
  if (crates.rc !== 0 || !crates.stdout.includes(`version: ${version}`)) failures.push('crates_beta15_7_missing');

  const report = {
    schemaVersion: 'brik64.cli_beta15_7_marketplace_package_gate.v1',
    version,
    pythonVersion: pyVersion,
    decision: failures.length === 0 ? 'PASS_BETA15_7_MARKETPLACE_PACKAGES' : 'FAIL_BETA15_7_MARKETPLACE_PACKAGES',
...
main().catch((error) => {
  ...
  fs.writeFileSync(path.join(outDir, 'report.json'), `${JSON.stringify({
    schemaVersion: 'brik64.cli_beta15_7_marketplace_package_gate.v1',
    version,
    decision: 'FAIL_BETA15_7_MARKETPLACE_PACKAGES',
