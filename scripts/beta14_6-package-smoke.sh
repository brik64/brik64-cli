...
PKG_DIR="$ROOT_DIR/evidence/beta14_6-package"
OUT_DIR="$ROOT_DIR/evidence/beta14_6-package-smoke"
...
[[ "$manifest_decision" == "PASS_BRIK64_CLI_BETA14_6_PACKAGE_BUILT" ]] || { echo "package_decision_drift:$manifest_decision" >&2; exit 1; }
...
if [[ ! -e "$EXTRACTED/evidence" ]]; then
  echo "package_missing_beta14_6_evidence_payload" >&2
  exit 1
fi
...
jq -e '.decision == "PASS_BETA14_5_128_EXECUTABLE_GATE"' "$EXTRACTED/evidence/beta14_5-128-executable/report.json" >/dev/null || {
  echo "package_beta14_6_128_executable_gate_not_pass" >&2
  exit 1
}
jq -e '.decision == "PASS_BETA14_5_FUNCTIONAL_CLOSURE_GATE"' "$EXTRACTED/evidence/beta14_5-functional-closure/report.json" >/dev/null || {
  echo "package_beta14_6_functional_closure_gate_not_pass" >&2
  exit 1
}
jq -e '.decision == "PASS_BETA14_6_DOMAIN_CONTRACTS_GATE"' "$EXTRACTED/evidence/beta14_6-domain-contracts/report.json" >/dev/null || {
  echo "package_beta14_6_domain_contracts_gate_not_pass" >&2
  exit 1
}
jq -e '.decision == "PASS_BETA14_3_L6_GENERATION_GATE"' "$EXTRACTED/evidence/beta14_3-l6-generation/gate-report.json" >/dev/null || {
  echo "package_prior_l6_gate_not_pass" >&2
  exit 1
}
...
JS
  ...
  cat > sample.rs <<'RS'
pub fn add_fee(x: i64) -> i64 { return x + 40; }
fn halve(value: i64) -> i64 { return value / 2; }
RS
  mkdir -p samples
  cp sample.js samples/sample.js
  cp sample.rs samples/sample.rs
  run_pass lift_rust "\"language\": \"rust\"" env BRIK64_CONFIG_HOME="$CONFIG_HOME" node "$BRIK" lift rust sample.rs --preview --json
  run_pass lift_js_dir "\"fileCount\":" env BRIK64_CONFIG_HOME="$CONFIG_HOME" node "$BRIK" lift js samples --preview --json
...
PCD
  ...
  cat > pcd/float.pcd <<'PCD'
PC float_gate {
  domain a: f64 [0, 1000];
  domain b: f64 [1, 1000];
  fn float_gate(a: f64, b: f64) -> f64 {
    return MC_64.FADD(a, b);
  }
}
PCD
  run_pass certify_float "certificate=pcd/float.pcd.cert.json" env BRIK64_CONFIG_HOME="$CONFIG_HOME" node "$BRIK" certify pcd/float.pcd
  run_pass emit_rust_float "generated=" env BRIK64_CONFIG_HOME="$CONFIG_HOME" node "$BRIK" emit pcd/float.pcd --target rust --out out-rust-float --tests
  rustc --test out-rust-float/program_test.rs -o out-rust-float/program_test >/dev/null 2>"$TMP_DIR/rust_float.stderr" || {
    cat "$TMP_DIR/rust_float.stderr" >&2
    exit 1
  }
  run_pass rust_generated "test result: ok" ./out-rust-float/program_test
...
jq -n \
  ...
  '{
    schemaVersion:"brik64.cli_beta14_6_package_smoke.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA14_6_LOCAL_PACKAGE_SMOKE",
...
printf 'decision=PASS_BRIK64_CLI_BETA14_6_LOCAL_PACKAGE_SMOKE\n'
printf 'checks=30\n'
