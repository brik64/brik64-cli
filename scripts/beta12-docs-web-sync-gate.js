...
const outDir = path.join(root, 'evidence', 'beta12-docs-web-sync');
...
function main() {
  ...
  checkFile('docs_sdks', path.join(docsRoot, 'sdks.mdx'), ['@brik64/core@0.1.0-beta.12', 'brik64==0.1.0b12', 'brik64-core@0.1.0-beta.12'], failures, artifacts);
  checkFile('web_release_data', path.join(webRoot, 'src/lib/language-data.ts'), ['@brik64/core@0.1.0-beta.12', 'brik64==0.1.0b12', 'brik64-core'], failures, artifacts);
  ...
  for (const stale of scan(docsRoot, '0\\.1\\.0-beta\\.(?:[4-9]|10|11)\\b|0\\.1\\.0b(?:[4-9]|10|11)\\b')) {
    failures.push(`docs_stale_beta_residue:${stale}`);
  }
  for (const stale of scan(webRoot, '0\\.1\\.0-beta\\.(?:[4-9]|10|11)\\b|0\\.1\\.0b(?:[4-9]|10|11)\\b')) {
    failures.push(`web_stale_beta_residue:${stale}`);
  }
  const report = {
    schemaVersion: 'brik64.cli_beta12_docs_web_sync_gate.v1',
    version,
    decision: failures.length === 0 ? 'PASS_DOCS_WEB_BETA12_SYNC' : 'FAIL_DOCS_WEB_BETA12_SYNC',
    releaseEligible: failures.length === 0,
    deployAllowed: failures.length === 0,
    artifacts,
    failures,
    boundary: 'Docs and web source must be Beta12 aligned before publication. This gate does not deploy.'
  };
