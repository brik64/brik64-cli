...
const evidenceDir = process.env.BRIK64_BETA11_ADVERSARIAL_EVIDENCE
  ? path.resolve(process.env.BRIK64_BETA11_ADVERSARIAL_EVIDENCE)
  : path.join(root, 'evidence', 'beta11-adversarial');
const reportPath = path.join(evidenceDir, 'report.json');
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'brik64-beta11-adversarial-'));
...
record('regression:beta11_semantic_polymerize_gate', () => {
  const result = expectPass('semantic-polymerize', 'npm', ['run', 'gate:beta11:semantic-polymerize']);
  return { stdoutSha256: sha256(result.stdout) };
});

record('regression:beta11_rust_emitter_clean_gate', () => {
  const result = expectPass('rust-emitter-clean', 'npm', ['run', 'gate:beta11:rust-emitter-clean']);
  return { stdoutSha256: sha256(result.stdout) };
});

record('regression:beta11_doctor_empty_workspace_gate', () => {
  const result = expectPass('doctor-empty-workspace', 'npm', ['run', 'gate:beta11:doctor-empty-workspace']);
  return { stdoutSha256: sha256(result.stdout) };
});
...
const decision = blockers.length === 0
  ? 'PASS_BETA11_ADVERSARIAL_GATE'
  : 'BLOCKED_BETA11_ADVERSARIAL_GATE';
const report = {
  schemaVersion: 'brik64.cli_beta11_adversarial_gate.v1',
