...
const outDir = path.join(root, 'evidence', 'beta6-skills-sync');
...
function main() {
  ...
  for (const rel of files) {
    ...
    if (/0\.1\.0-beta\.[45]|0\.1\.0b[45]|beta[45]|beta\.[45]/.test(text)) failures.push(`skill_stale_beta_residue:${rel}`);
  ...
  if (!brik64.includes('version: 0.1.0-beta.6')) failures.push('brik64_skill_beta6_version_missing');
  if (!brik64.includes('Public CLI version: `0.1.0-beta.6`')) failures.push('brik64_skill_public_version_boundary_missing');

  const report = {
    schemaVersion: 'brik64.cli_beta6_skills_sync_gate.v1',
    version: '0.1.0-beta.6',
    decision: failures.length === 0 ? 'PASS_SKILLS_BETA6_SYNC' : 'FAIL_SKILLS_BETA6_SYNC',
    releaseEligible: failures.length === 0,
    publicSkillPublicationAllowed: failures.length === 0,
    skillsRoot,
    artifacts,
    failures,
    boundary: 'Public skills are beta6 aligned and must not expose private release or engine nomenclature.'
  };
