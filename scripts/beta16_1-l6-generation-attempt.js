...
if (!/^0\.1\.0-beta\.16\.1$/.test(version)) {
  throw new Error(`unsupported_beta16_1_family_version:${version}`);
}
...
const inputPcdPaths = [
  'pcd/beta16_1/release/l6_cli_materialization_contract.pcd',
  'pcd/beta16_1/release/l6_cli_materialization_result_contract.pcd',
...
const outputRefs = {
  generatedArtifact: 'evidence/beta16_1-l6-generation/generated/brik64-cli.mjs',
  package: `evidence/beta16_1-package/brik64-cli-${version}.tgz`,
  releaseManifest: 'release/manifest.json',
  sealReport: 'evidence/beta16_1-l6-generation/seal_report.json',
};
...
function buildRequest(inputs, pcdInputSetSha256, blockers) {
  ...
  return {
    ...
    version,
    lane: 'cli_0_1_beta',
    iterId: 'beta16.1-l6-cli-materializer-request',
...
function materializationAttempts(requestLinePath, context) {
  ...
  return ['l6-cli-materialize', 'beta16.1-cli-materialize', 'materialize', 'compile'].map((command) => {
    const remote = [
      'set -u',
      'tmp="$(mktemp /tmp/brik64-beta16-1-materializer-request.XXXXXX.json)"',
    ...
    return {
      command: [wrapper, command, '@@FILE:<beta16.1-materializer-request>'],
...
function main() {
  ...
  writeJson(requestManifestPath, {
    ...
    decision: blockers.length === 0
      ? 'PASS_BETA16_1_L6_MATERIALIZER_REQUEST_BUNDLE'
      : 'BLOCKED_BETA16_1_L6_MATERIALIZER_REQUEST_BUNDLE',
    request: {
  ...
  writeJson(path.join(evidenceDir, 'l6plus_engine_manifest.json'), {
    schemaVersion: 'brik64.cli_beta16_1_l6plus_engine_probe.v1',
    version,
  ...
  writeJson(path.join(evidenceDir, 'generated_artifact_manifest.json'), {
    schemaVersion: 'brik64.cli_beta16_1_l6_generated_artifact_manifest.v1',
    version,
    decision: materialization
      ? 'PASS_BETA16_1_L6_ARTIFACT_MATERIALIZATION'
      : 'BLOCKED_BETA16_1_L6_ARTIFACT_MATERIALIZATION',
  ...
  writeJson(path.join(evidenceDir, 'package.manifest.json'), {
    schemaVersion: 'brik64.cli_beta16_1_l6_package_manifest.v1',
    version,
    decision: materialization
      ? 'PASS_BETA16_1_L6_PACKAGE_MANIFEST'
      : 'BLOCKED_BETA16_1_L6_PACKAGE_MANIFEST',
  ...
  writeJson(path.join(evidenceDir, 'seal_report.json'), {
    ...
    version,
    decision: materialization ? 'PASS_BETA16_1_L6_SEAL' : 'BLOCKED_BETA16_1_L6_SEAL',
  ...
  writeJson(path.join(evidenceDir, 'hashes.json'), {
    schemaVersion: 'brik64.cli_beta16_1_l6_hashes.v1',
    version,
    inputPcds: inputs,
    pcdInputSetSha256,
  ...
  const gateReport = {
    schemaVersion: 'brik64.cli_beta16_1_l6_generation_gate.v1',
    version,
    generatedAt: new Date().toISOString(),
    decision: materialization ? 'PASS_BETA16_1_L6_GENERATION_GATE' : 'BLOCKED_BETA16_1_L6_GENERATION_GATE',
    ...
    nextAction: materialization
      ? 'bind Beta16.1 package and release manifest to public release train'
      : 'build Beta16.1 package and release manifest, then expose L6+N5 CLI artifact materializer for PCD/polymer -> artifact -> package -> release manifest',
  };
