...
const evidenceDir = path.join(root, 'evidence', 'beta15_7-pre-public-rc');
const records = [];
...
try {
  ...
  run('node_check_beta15_7_semantic_gate', process.execPath, ['--check', 'scripts/beta15_7-semantic-correctness-gate.js']);
  run('node_check_beta15_7_gate', process.execPath, ['--check', 'scripts/beta15_7-rust-f64-command-lift-gate.js']);
  run('node_check_self', process.execPath, ['--check', 'scripts/beta15_7-pre-public-rc-gate.js']);
  for (const pcd of [
  ...
  run('gate_beta15_7_rust_f64_command_lift', process.execPath, ['scripts/beta15_7-rust-f64-command-lift-gate.js'], {
    expect: 'PASS_BRIK64_CLI_BETA15_7_RUST_F64_COMMAND_LIFT_GATE'
  });
  run('gate_beta15_7_semantic_correctness', process.execPath, ['scripts/beta15_7-semantic-correctness-gate.js'], {
    expect: 'PASS_BRIK64_CLI_BETA15_7_SEMANTIC_CORRECTNESS_GATE'
  });
  ...
  const gate = readJson('evidence/beta15_7-rust-f64-command-lift/report.json');
  assert(gate.decision === 'PASS_BRIK64_CLI_BETA15_7_RUST_F64_COMMAND_LIFT_GATE', 'beta15_7_gate_not_pass', gate);
  const report = {
    schemaVersion: 'brik64.cli_beta15_7_pre_public_rc_report.v1',
    version,
    decision: 'PASS_BRIK64_CLI_BETA15_7_PRE_PUBLIC_RC_GATE',
    ...
    checked: [
      ...
      'beta15_7_rust_f64_command_lift',
      'beta15_7_semantic_correctness',
      'smoke',
  ...
  process.stdout.write('decision=PASS_BRIK64_CLI_BETA15_7_PRE_PUBLIC_RC_GATE\n');
} catch (error) {
  const report = {
    schemaVersion: 'brik64.cli_beta15_7_pre_public_rc_report.v1',
    version,
    decision: 'FAIL_BRIK64_CLI_BETA15_7_PRE_PUBLIC_RC_GATE',
    evidenceLevel: 'NIVEL 3',
    releaseEligible: false,
    error: error.message,
