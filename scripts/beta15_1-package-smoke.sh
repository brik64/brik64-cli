...
PKG_DIR="$ROOT_DIR/evidence/beta15_1-package"
OUT_DIR="$ROOT_DIR/evidence/beta15_1-package-smoke"
...
[[ "$manifest_decision" == "PASS_BRIK64_CLI_BETA15_1_PACKAGE_BUILT" ]] || { echo "package_decision_drift:$manifest_decision" >&2; exit 1; }
[[ "$release_eligible" == "false" ]] || { echo "beta15_1_candidate_should_not_be_public_release_eligible" >&2; exit 1; }
...
[[ -f "$EXTRACTED/evidence/beta15_1-ledger-dtl/report.json" ]] || {
  echo "package_missing_beta15_1_ledger_evidence" >&2
  exit 1
}
jq -e '.decision == "PASS_LOCAL_CANDIDATE_GATE"' "$EXTRACTED/evidence/beta15_1-ledger-dtl/report.json" >/dev/null || {
  echo "package_beta15_1_ledger_gate_not_pass" >&2
  exit 1
}
[[ -f "$EXTRACTED/pcd/beta15_1/cli/ledger_commands.pcd" ]] || {
  echo "package_missing_beta15_1_pcd_contracts" >&2
  exit 1
}
...
jq -n \
  --arg version "$VERSION" \
  '{
    schemaVersion:"brik64.cli_beta15_1_package_smoke_report.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA15_1_PACKAGE_SMOKE",
    releaseEligible:false,
    checked:[
      "package_sha",
      "embedded_beta15_1_evidence",
      "version",
      "ledger_help",
      "certify",
...
printf 'decision=PASS_BRIK64_CLI_BETA15_1_PACKAGE_SMOKE\n'
