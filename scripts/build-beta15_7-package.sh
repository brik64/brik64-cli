...
OUT_DIR="$ROOT_DIR/evidence/beta15_7-package"
...
pre_public_gate="$ROOT_DIR/evidence/beta15_7-pre-public-rc/report.json"
...
[[ "$pre_public_gate_decision" == "PASS_BRIK64_CLI_BETA15_7_PRE_PUBLIC_RC_GATE" ]] || failures+=("beta15_7_pre_public_gate_missing_or_invalid:$pre_public_gate_decision")
...
if [[ "${#failures[@]}" -gt 0 ]]; then
  ...
  printf 'beta15_7_package_input_gate_failed:%s\n' "${failures[*]}" >&2
  exit 1
fi
...
jq -n \
  --arg version "$VERSION" \
  --arg packagePath "evidence/beta15_7-package/$PACKAGE_NAME" \
  ...
  '{
    ...
    inputGates:[{
      ...
      report:"evidence/beta15_7-pre-public-rc/report.json",
      packaged:false
    },{
      decision:"PASS_BRIK64_CLI_BETA15_7_RUST_F64_COMMAND_LIFT_GATE",
      report:"evidence/beta15_7-rust-f64-command-lift/report.json",
      packaged:false,
      blocker:null
    },{
    ...
    requiredPublicReleaseGates:[
      "beta15_7_package_smoke",
      "github_release_beta15_7",
      "curl_gcp_installer_beta15_7",
      "web_docs_changelog_beta15_7",
      "skills_beta15_7",
      "sdk_beta15_7_marketplaces_or_deferrals",
      "public_claim_scan",
      "live_release_train_verify"
    ],
    boundary:"Beta15.7 public beta package. This manifest describes package assembly and does not assert formal correctness, self-hosting, fixpoint, or toolchain independence."
  }' > "$MANIFEST_PATH"
...
printf 'decision=PASS_BRIK64_CLI_BETA15_7_PACKAGE_BUILT\n'
printf 'releaseEligible=true\n'
printf 'package=evidence/beta15_7-package/%s\n' "$PACKAGE_NAME"
printf 'sha256=%s\n' "$package_sha"
