...
OUT_DIR="$ROOT_DIR/evidence/beta12-package"
...
local_gate="$ROOT_DIR/evidence/beta12-adversarial/report.json"
...
[[ "$local_gate_decision" == "PASS_BETA12_ADVERSARIAL_GATE" ]] || failures+=("beta12_adversarial_gate_missing_or_invalid:$local_gate_decision")
...
if [[ "${#failures[@]}" -gt 0 ]]; then
  ...
  printf '%s\n' "${failures[@]}" | jq -Rsc \
    --arg version "$VERSION" \
    '{
      schemaVersion:"brik64.cli_beta12_package_manifest.v1",
      version:$version,
      decision:"FAIL_BRIK64_CLI_BETA12_PACKAGE_BUILT",
      releaseEligible:false,
      failures:(split("\n") | map(select(length > 0)))
    }' > "$MANIFEST_PATH"
  printf 'beta12_package_input_gate_failed:%s\n' "${failures[*]}" >&2
  exit 1
fi
...
jq -n \
  ...
  '{
    ...
    version:$version,
    private:true,
    description:"BRIK64 CLI beta12 candidate for secure local PCD workflows, SDK parity checks, and opt-in telemetry transport.",
...
jq -n \
  --arg version "$VERSION" \
  --arg packagePath "evidence/beta12-package/$PACKAGE_NAME" \
  --arg packageSha "$package_sha" \
  --argjson packageBytes "$(file_size "$PACKAGE_PATH")" \
  '{
    schemaVersion:"brik64.cli_beta12_package_manifest.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA12_PACKAGE_BUILT",
    releaseEligible:false,
    lane:"cli_0_1_beta12",
    generationClaim:"assisted_generation_non_claim",
    package:{path:$packagePath, sha256:$packageSha, bytes:$packageBytes},
    inputGates:[{
      decision:"PASS_BETA12_ADVERSARIAL_GATE",
      report:"evidence/beta12-adversarial/report.json",
      packaged:false
    }],
    requiredPublicReleaseGates:[
      "beta12_package_smoke",
      "beta12_github_release",
      "curl_gcp_installer_beta12",
      "web_docs_changelog_beta12",
      "skills_beta12",
      "sdk_beta12_marketplaces_or_deferrals",
      "public_claim_scan",
      "live_release_train_verify"
    ],
    boundary:"Beta12 local package candidate only. Public release remains blocked until GitHub, curl/GCP, docs, web, changelog, SDK, skills, telemetry transport and live verification pass atomically."
  }' > "$MANIFEST_PATH"
...
printf 'decision=PASS_BRIK64_CLI_BETA12_PACKAGE_BUILT\n'
printf 'releaseEligible=false\n'
printf 'package=evidence/beta12-package/%s\n' "$PACKAGE_NAME"
printf 'sha256=%s\n' "$package_sha"
