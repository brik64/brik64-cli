...
const label = 'beta17-fixpoint-materializer-generation-request';
...
const outputRefs = {
  generatedMaterializer: 'generated/beta17-fixpoint-stage-materializer.js',
  materializerProvenance: 'evidence/beta17-fixpoint-remote-dispatcher/materializer-provenance.json',
  generationReport: 'evidence/beta17-fixpoint-materializer-generation/generation-report.json',
};
...
function requestLineSha256(request) {
  return sha256(`BRIK64_BETA17_FIXPOINT_MATERIALIZER_GENERATION_REQUEST\t${Buffer.from(JSON.stringify(request)).toString('base64')}\n`);
}
...
function buildRequest(options = {}) {
  ...
  return {
    schemaVersion: 'brik64.beta17_fixpoint_materializer_generation_request.v1',
    version,
    lane: 'l6plus_n5_self_host_fixpoint',
    iterId: 'beta17-fixpoint-materializer-generation-request',
    requestType: 'generate_beta17_fixpoint_stage_materializer',
    ...
    requiredResultSchema: 'brik64.beta17_fixpoint_materializer_generation_result.v1',
    requiredResultVersion: version,
    requiredGeneratedMaterializerMarker: 'BRIK64_BETA17_FIXPOINT_STAGE_RESULT',
...
function validateRequest(request) {
  ...
  if (request?.schemaVersion !== 'brik64.beta17_fixpoint_materializer_generation_request.v1') blockers.push('request_schema_invalid');
  ...
  if (request?.requestType !== 'generate_beta17_fixpoint_stage_materializer') blockers.push('request_type_invalid');
  ...
  if (request?.requiredGeneratedMaterializerMarker !== 'BRIK64_BETA17_FIXPOINT_STAGE_RESULT') {
    blockers.push('request_generated_materializer_marker_invalid');
  }
...
function writeBundle() {
  ...
  fs.writeFileSync(requestLinePath, `BRIK64_BETA17_FIXPOINT_MATERIALIZER_GENERATION_REQUEST\t${encoded}\n`);
  ...
  const manifest = {
    schemaVersion: 'brik64.beta17_fixpoint_materializer_generation_request_manifest.v1',
    version,
    decision: 'PASS_BETA17_FIXPOINT_MATERIALIZER_GENERATION_REQUEST_BUNDLE',
    status: 'REQUEST_NON_CLAIM',
    request: {
...
function verifyBundle() {
  ...
  const validation = validateRequest(request);
  console.log(validation.accepted ? 'PASS beta17 fixpoint materializer generation request bundle' : 'FAIL beta17 fixpoint materializer generation request bundle');
...
if (require.main === module) {
  try {
    ...
    console.error(`beta17_materializer_generation_request_fail_closed:${error.message}`);
    process.exit(2);
  }
}
