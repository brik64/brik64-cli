...
OUT_DIR="$ROOT_DIR/evidence/beta13-package"
...
local_gate="$ROOT_DIR/evidence/beta13-source-lift/report.json"
...
[[ "$local_gate_decision" == "PASS_BETA13_SOURCE_LIFT_GATE" ]] || failures+=("beta13_source_lift_gate_missing_or_invalid:$local_gate_decision")
...
if [[ "${#failures[@]}" -gt 0 ]]; then
  ...
  printf '%s\n' "${failures[@]}" | jq -Rsc \
    --arg version "$VERSION" \
    '{
      schemaVersion:"brik64.cli_beta13_package_manifest.v1",
      version:$version,
      decision:"FAIL_BRIK64_CLI_BETA13_PACKAGE_BUILT",
      releaseEligible:false,
      failures:(split("\n") | map(select(length > 0)))
    }' > "$MANIFEST_PATH"
  printf 'beta13_package_input_gate_failed:%s\n' "${failures[*]}" >&2
  exit 1
fi
...
inputs=(
  ...
  "pcd/cli_beta13_source_lift_contract.pcd"
...
jq -n \
  ...
  '{
    ...
    version:$version,
    private:true,
    description:"BRIK64 CLI beta13 candidate for local source lift preview, adoption reports, and secure PCD workflows.",
...
jq -n \
  --arg version "$VERSION" \
  --arg packagePath "evidence/beta13-package/$PACKAGE_NAME" \
  --arg packageSha "$package_sha" \
  --argjson packageBytes "$(file_size "$PACKAGE_PATH")" \
  '{
    schemaVersion:"brik64.cli_beta13_package_manifest.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA13_PACKAGE_BUILT",
    releaseEligible:false,
    lane:"cli_0_1_beta13",
    generationClaim:"assisted_generation_non_claim",
    package:{path:$packagePath, sha256:$packageSha, bytes:$packageBytes},
    inputGates:[{
      decision:"PASS_BETA13_SOURCE_LIFT_GATE",
      report:"evidence/beta13-source-lift/report.json",
      packaged:false
    }],
    requiredPublicReleaseGates:[
      "beta13_package_smoke",
      "beta13_github_release",
      "curl_gcp_installer_beta13",
      "web_docs_changelog_beta13",
      "skills_beta13",
      "sdk_beta13_marketplaces_or_deferrals",
      "public_claim_scan",
      "live_release_train_verify"
    ],
    boundary:"Beta13 local package candidate only. Public release remains blocked until GitHub, curl/GCP, docs, web, changelog, SDK, skills, marketplace publication and live verification pass atomically."
  }' > "$MANIFEST_PATH"
...
printf 'decision=PASS_BRIK64_CLI_BETA13_PACKAGE_BUILT\n'
printf 'releaseEligible=false\n'
printf 'package=evidence/beta13-package/%s\n' "$PACKAGE_NAME"
printf 'sha256=%s\n' "$package_sha"
