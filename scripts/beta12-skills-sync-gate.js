...
const outDir = path.join(root, 'evidence', 'beta12-skills-sync');
...
function main() {
  ...
  for (const needle of [version, '@brik64/core@0.1.0-beta.12', 'brik64==0.1.0b12', 'brik64-core@0.1.0-beta.12']) {
  ...
  const report = {
    schemaVersion: 'brik64.cli_beta12_skills_sync_gate.v1',
    version,
    decision: failures.length === 0 ? 'PASS_SKILLS_BETA12_SYNC' : 'FAIL_SKILLS_BETA12_SYNC',
    releaseEligible: failures.length === 0,
    publicSkillPublicationAllowed: failures.length === 0,
    skillsRoot,
    artifacts,
    failures,
    boundary: 'Public skills must be Beta12 aware and must not expose private engine nomenclature.'
  };
