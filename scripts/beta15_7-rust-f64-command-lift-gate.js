...
const evidenceDir = path.join(root, 'evidence', 'beta15_7-rust-f64-command-lift');
...
try {
  ...
  write('adversarial/bad.pcd', '// brik64.pcd_file.v1\n// claim_boundary: local_candidate_only\nPC bad { domain x: i64 [0, 99999999999999999999]; fn bad(x: i64) -> i64 { return x; } }\n');
  ...
  const report = {
    schemaVersion: 'brik64.cli_beta15_7_rust_f64_command_lift_gate.v1',
    version,
  ...
  process.stdout.write('decision=PASS_BRIK64_CLI_BETA15_7_RUST_F64_COMMAND_LIFT_GATE\n');
} catch (error) {
  const report = {
    schemaVersion: 'brik64.cli_beta15_7_rust_f64_command_lift_gate.v1',
    version,
    decision: 'FAIL_BRIK64_CLI_BETA15_7_RUST_F64_COMMAND_LIFT_GATE',
    evidenceLevel: 'NIVEL 3',
    releaseEligible: false,
    error: error.message,
