...
[[ "$release_eligible" == "false" ]] || { echo "beta15_7_source_candidate_package_must_not_be_release_eligible" >&2; exit 1; }
[[ "$publication_allowed" == "false" ]] || { echo "beta15_7_source_candidate_package_must_not_allow_publication" >&2; exit 1; }
[[ "$public_release_allowed" == "false" ]] || { echo "beta15_7_source_candidate_claim_boundary_open" >&2; exit 1; }
...
pushd "$WORK_DIR" >/dev/null
  ...
  mkdir -p pcd
  cat > pcd/beta15_7_smoke.pcd <<'PCD'
// brik64.pcd_file.v1
// claim_boundary: local_candidate_only
PC beta15_7_smoke {
  domain input: i64 [0, 255];
  fn beta15_7_smoke(input: i64) -> i64 {
    if (input == 0) return 1;
    return 2;
  }
}
PCD
  run_pass certify "certificate=pcd/beta15_7_smoke.pcd.cert.json" env BRIK64_NO_BANNER=1 node "$BRIK" certify pcd/beta15_7_smoke.pcd --prototype-non-claim
  run_pass verify '"status": "PASS"' env BRIK64_NO_BANNER=1 node "$BRIK" verify pcd/beta15_7_smoke.pcd --json
  run_pass emit_python "tests=out/tests/test_beta15_7_smoke.py" env BRIK64_NO_BANNER=1 node "$BRIK" emit pcd/beta15_7_smoke.pcd --target python --out out --tests
  test -f out/brik64_generated_beta15_7_smoke/program.py
  test -f out/tests/test_beta15_7_smoke.py
