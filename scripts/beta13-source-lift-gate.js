...
const evidenceDir = path.join(root, 'evidence', 'beta13-source-lift');
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'brik64-beta13-'));
const checks = [];
const failures = [];
...
const report = {
  schemaVersion: 'brik64.cli_beta13_source_lift_gate.v1',
  version,
  decision: failures.length === 0 ? 'PASS_BETA13_SOURCE_LIFT_GATE' : 'FAIL_BETA13_SOURCE_LIFT_GATE',
  releaseEligible: failures.length === 0,
  evidenceLevel: failures.length === 0 ? 3 : 1,
  checks,
  failures,
  boundary: 'Beta13 source lift is local preview only. It generates PCD candidates, not certificates or formal correctness claims.'
};
