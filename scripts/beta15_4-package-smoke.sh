...
PKG_DIR="$ROOT_DIR/evidence/beta15_4-package"
OUT_DIR="$ROOT_DIR/evidence/beta15_4-package-smoke"
...
[[ "$manifest_decision" == "PASS_BRIK64_CLI_BETA15_4_PACKAGE_BUILT" ]] || { echo "package_decision_drift:$manifest_decision" >&2; exit 1; }
[[ "$release_eligible" == "false" ]] || { echo "beta15_4_candidate_should_not_be_public_release_eligible" >&2; exit 1; }
...
[[ -f "$EXTRACTED/evidence/beta15_4-pre-public-rc/report.json" ]] || {
  echo "package_missing_beta15_4_pre_public_evidence" >&2
  exit 1
}
jq -e '.decision == "PASS_BRIK64_CLI_BETA15_4_PRE_PUBLIC_RC_GATE"' "$EXTRACTED/evidence/beta15_4-pre-public-rc/report.json" >/dev/null || {
  echo "package_beta15_4_pre_public_gate_not_pass" >&2
  exit 1
}
[[ -f "$EXTRACTED/pcd/beta15_4/cli/rust_app_polymer_domain_codegen.pcd" ]] || {
  echo "package_missing_beta15_4_pcd_contracts" >&2
  exit 1
}
[[ -f "$EXTRACTED/evidence/beta15_4-rust-polymer-domain/report.json" ]] || {
  echo "package_missing_beta15_4_rust_polymer_domain_evidence" >&2
  exit 1
}
jq -e '.decision == "PASS_BRIK64_CLI_BETA15_4_RUST_POLYMER_DOMAIN_GATE"' "$EXTRACTED/evidence/beta15_4-rust-polymer-domain/report.json" >/dev/null || {
  echo "package_beta15_4_rust_polymer_domain_gate_not_pass" >&2
  exit 1
}
...
jq -n \
  --arg version "$VERSION" \
  '{
    schemaVersion:"brik64.cli_beta15_4_package_smoke_report.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA15_4_PACKAGE_SMOKE",
    releaseEligible:false,
    checked:[
      "package_sha",
      "embedded_beta15_4_evidence",
      "embedded_beta15_4_rust_polymer_domain_gate",
      "version",
      "ledger_help",
      "certify",
...
printf 'decision=PASS_BRIK64_CLI_BETA15_4_PACKAGE_SMOKE\n'
