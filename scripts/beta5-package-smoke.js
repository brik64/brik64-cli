...
const outDir = path.join(root, 'evidence', 'beta5-package-smoke');
const pkgDir = path.join(root, 'evidence', 'beta5-package');
...
function main() {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'brik64-beta5-package-smoke-'));
  ...
  requirePass('version', ['node', brik, '--version'], extracted, 'BRIK64 CLI 0.1.0-beta.5');
  ...
  const report = {
    schemaVersion: 'brik64.cli_beta5_package_smoke.v1',
    version: '0.1.0-beta.5',
    decision: 'PASS_LOCAL_PACKAGE_SMOKE',
    releaseEligible: false,
    package: manifest.package,
    checks: ['extract', 'version', 'engine-status', 'doctor', 'init', 'certify', 'emit-ts', 'stale-cert-fail-closed'],
    boundary: 'Local package smoke only. Cross-platform release smoke remains separate.'
  };
