...
const evidenceDir = path.join(root, 'evidence', 'beta14_5-monomer-128');
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'brik64-beta14-4-'));
const checks = [];
const failures = [];
...
record('pcd:source_scaffold_present', () => {
  const base = path.join(root, 'pcd', 'beta14_5');
  const required = [
    ...
    'release/beta14_5_release_train.pcd'
  ];
...
record('l6:prior_baseline_packaged_without_new_claim', () => {
  ...
  const gatePath = path.join(root, 'evidence', 'beta14_3-l6-generation', 'gate-report.json');
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  const seal = JSON.parse(fs.readFileSync(sealPath, 'utf8'));
  const gate = JSON.parse(fs.readFileSync(gatePath, 'utf8'));
  assert(
    gate.decision === 'PASS_BETA14_3_L6_GENERATION_GATE',
    'prior_l6_gate_invalid',
    gate
  );
  assert(
    seal.decision === 'PASS_BETA14_3_L6_SEAL',
    'prior_l6_seal_invalid',
    seal
  );
  assert(manifest.version === '0.1.0-beta.14.3', 'prior_l6_manifest_version_drift', manifest);
  return { priorL6Baseline: 'beta14_3', claimBoundary: 'not_new_beta14_5_l6_materialization' };
});

fs.mkdirSync(evidenceDir, { recursive: true });
const report = {
  schemaVersion: 'brik64.cli_beta14_5_monomer_128_gate.v1',
  version,
  decision: failures.length === 0 ? 'PASS_BETA14_5_MONOMER_128_GATE' : 'FAIL_BETA14_5_MONOMER_128_GATE',
  evidenceLevel: failures.length === 0 ? 3 : 1,
  checks,
  failures,
  boundary: 'Registry, parser and source-scaffold gate only. Prior Beta14.3 L6 evidence is consumed as baseline; this is not fresh Beta14.5 L6 generation evidence, self-hosting, fixpoint, formal proof, or public release authorization.'
};
