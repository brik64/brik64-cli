...
OUT_DIR="$ROOT_DIR/evidence/beta15_4-package"
...
pre_public_gate="$ROOT_DIR/evidence/beta15_4-pre-public-rc/report.json"
...
[[ "$pre_public_gate_decision" == "PASS_BRIK64_CLI_BETA15_4_PRE_PUBLIC_RC_GATE" ]] || failures+=("beta15_4_pre_public_gate_missing_or_invalid:$pre_public_gate_decision")
...
if [[ "${#failures[@]}" -gt 0 ]]; then
  ...
  printf '%s\n' "${failures[@]}" | jq -Rsc \
    --arg version "$VERSION" \
    '{
      schemaVersion:"brik64.cli_beta15_4_package_manifest.v1",
      version:$version,
      decision:"FAIL_BRIK64_CLI_BETA15_4_PACKAGE_BUILT",
      releaseEligible:false,
      failures:(split("\n") | map(select(length > 0)))
    }' > "$MANIFEST_PATH"
  printf 'beta15_4_package_input_gate_failed:%s\n' "${failures[*]}" >&2
  exit 1
fi
...
inputs=(
  ...
  "pcd/beta15_3"
  "pcd/beta15_4"
  "evidence/beta15_4-pre-public-rc/report.json"
  "evidence/beta15_4-rust-polymer-domain/report.json"
  "evidence/beta15_4-stale-public-surface/report.json"
...
jq -n \
  --arg version "$VERSION" \
  --arg packagePath "evidence/beta15_4-package/$PACKAGE_NAME" \
  ...
  '{
    ...
    inputGates:[{
      ...
      report:"evidence/beta15_4-pre-public-rc/report.json",
      packaged:true
    },{
      decision:"PASS_BRIK64_CLI_BETA15_4_RUST_POLYMER_DOMAIN_GATE",
      report:"evidence/beta15_4-rust-polymer-domain/report.json",
      packaged:true,
      blocker:null
    },{
      ...
      packaged:false,
      blocker:"beta15_4_l6_generation_evidence_pending"
    }],
    requiredPublicReleaseGates:[
      "beta15_4_package_smoke",
      "github_release_beta15_4",
      "curl_gcp_installer_beta15_4",
      "web_docs_changelog_beta15_4",
      "skills_beta15_4",
      "sdk_beta15_4_marketplaces_or_deferrals",
      "public_claim_scan",
      "live_release_train_verify"
    ],
    boundary:"Beta15.4 local package candidate only. Public release remains blocked until L6 generation evidence, package smoke, GitHub, curl/GCP, docs, web, changelog, SDK, skills, marketplace publication or deferral, and live verification pass atomically."
  }' > "$MANIFEST_PATH"
...
printf 'decision=PASS_BRIK64_CLI_BETA15_4_PACKAGE_BUILT\n'
printf 'releaseEligible=false\n'
printf 'package=evidence/beta15_4-package/%s\n' "$PACKAGE_NAME"
printf 'sha256=%s\n' "$package_sha"
