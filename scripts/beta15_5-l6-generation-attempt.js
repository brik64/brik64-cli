...
} = require('./beta15_5-l6-materialization-result');
const {
  buildRequest,
  validateRequest
} = require('./beta15_5-l6-materializer-request-bundle');

const root = path.resolve(__dirname, '..');
const version = '0.1.0-beta.15.5';
const label = 'beta15_5';
...
const inputPcds = [
  'pcd/beta15_5/release/l6_cli_materialization_contract.pcd',
  'pcd/beta15_5/release/l6_cli_materialization_result_contract.pcd',
  'pcd/beta15_5/cli/rust_f64_polymer_codegen.pcd',
  'pcd/beta15_5/harness/lift_roundtrip_gate.pcd',
  'pcd/beta15_5/release/public_surface_sync.pcd',
  'pcd/cli_core.pcd',
  'pcd/cli_polymer.pcd'
];
...
function technicalSheet(sourceHash, sourcePath) {
  const base = {
    ...
    boundedDomains: {
      ...
      cli_scope_beta15_5: { kind: 'integer', min: 0, max: 1 },
    ...
    normalization: {
      status: 'finite_beta15_5_materialization_domains',
    ...
    certificationLane: 'INTERNAL_BETA15_5_L6_MATERIALIZATION',
...
function directRoute2Materialization(inputs, expectedContext, remoteRefs) {
  ...
  const packageRef = fileRef('evidence/beta15_5-package/brik64-cli-0.1.0-beta.15.5.tgz');
  const sealPath = 'evidence/beta15_5-l6-generation/seal_report.json';
  const generatedArtifactRef = fileRef('evidence/beta15_5-l6-generation/materialization-out.tgz');
  ...
  writeJson(path.join(root, sealPath), {
    schemaVersion: 'brik64.cli_beta15_5_l6_seal_report.v1',
    version,
    decision: summary.failures === 0 ? 'PASS_BETA15_5_L6_SEAL' : 'BLOCKED_BETA15_5_L6_SEAL',
    compositeSha256,
    generationTraceSha256,
  ...
  return {
    schemaVersion: 'brik64.beta15_5_cli_l6_materialization_result.v1',
    version,
...
function main() {
  ...
  writeJson(path.join(evidenceDir, 'l6plus_engine_manifest.json'), {
    schemaVersion: 'brik64.cli_beta15_5_l6plus_engine_probe.v1',
    version,
  ...
  writeJson(path.join(evidenceDir, 'generated_artifact_manifest.json'), {
    schemaVersion: 'brik64.cli_beta15_5_l6_generated_artifact_manifest.v1',
    version,
    decision: materialization
      ? 'PASS_BETA15_5_L6_ARTIFACT_MATERIALIZATION'
      : 'BLOCKED_BETA15_5_L6_ARTIFACT_MATERIALIZATION',
    ...
    requiredNextAction: materialization
      ...
      : 'add or expose a Beta15.5 CLI PCD/polymer materialization endpoint, then rerun generation from canonical PCD inputs',
    inputPcds: materialization?.inputPcds || inputs
  });

  writeJson(path.join(evidenceDir, 'package.manifest.json'), {
    schemaVersion: 'brik64.cli_beta15_5_l6_package_manifest.v1',
    version,
    decision: materialization
      ? 'PASS_BETA15_5_L6_PACKAGE_MANIFEST'
      : 'BLOCKED_BETA15_5_L6_PACKAGE_MANIFEST',
  ...
  writeJson(path.join(evidenceDir, 'seal_report.json'), {
    schemaVersion: 'brik64.cli_beta15_5_l6_seal_report.v1',
    version,
    decision: materialization ? 'PASS_BETA15_5_L6_SEAL' : 'BLOCKED_BETA15_5_L6_SEAL',
  ...
  writeJson(path.join(evidenceDir, 'hashes.json'), {
    schemaVersion: 'brik64.cli_beta15_5_l6_hashes.v1',
    version,
    inputPcds: inputs,
    pcdInputSetSha256,
  ...
  const gateReport = {
    schemaVersion: 'brik64.cli_beta15_5_l6_generation_gate.v1',
    version,
    generatedAt: new Date().toISOString(),
    decision: materialization ? 'PASS_BETA15_5_L6_GENERATION_GATE' : 'BLOCKED_BETA15_5_L6_GENERATION_GATE',
    ...
    nextAction: 'implement or expose L6+N5 CLI artifact materializer for PCD/polymer -> artifact -> package -> release manifest before Beta15.5 publication'
  };
