...
const outDir = path.join(root, 'evidence', 'beta15_5-marketplace-packages');
...
async function main() {
  ...
  if (npm.rc !== 0 || npm.stdout.trim() !== version) failures.push('npm_beta15_5_missing');

  try {
    const pypi = await fetchJson('https://pypi.org/pypi/brik64/json');
    if (!pypi.releases || !pypi.releases[pyVersion]) failures.push('pypi_beta15_5_missing');
  } catch (error) {
    failures.push(`pypi_lookup_failed:${error.message}`);
  }

  const crates = run('cargo', ['info', `brik64-core@${version}`]);
  if (crates.rc !== 0 || !crates.stdout.includes(`version: ${version}`)) failures.push('crates_beta15_5_missing');

  const report = {
    schemaVersion: 'brik64.cli_beta15_5_marketplace_package_gate.v1',
    version,
    pythonVersion: pyVersion,
    decision: failures.length === 0 ? 'PASS_BETA15_5_MARKETPLACE_PACKAGES' : 'FAIL_BETA15_5_MARKETPLACE_PACKAGES',
...
main().catch((error) => {
  ...
  fs.writeFileSync(path.join(outDir, 'report.json'), `${JSON.stringify({
    schemaVersion: 'brik64.cli_beta15_5_marketplace_package_gate.v1',
    version,
    decision: 'FAIL_BETA15_5_MARKETPLACE_PACKAGES',
