...
const evidenceDir = path.join(root, 'evidence', 'beta14_4-128-executable');
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'brik64-beta14_4-128-'));
const checks = [];
const failures = [];
...
function record(id, fn) {
  try {
    const detail = fn() || {};
    checks.push({ id, status: 'PASS', elapsedMs: 0, ...detail });
  } catch (error) {
...
function fixtureFor(monomer) {
  ...
  return [
    '// brik64.pcd_file.v1',
    `// generated_by: brik64-cli ${version} beta14.4 128 executable gate`,
...
const report = {
  schemaVersion: 'brik64.beta14_4_128_executable_gate.v1',
  version,
  decision: failures.length === 0 ? 'PASS_BETA14_4_128_EXECUTABLE_GATE' : 'FAIL_BETA14_4_128_EXECUTABLE_GATE',
