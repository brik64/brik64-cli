...
PKG_DIR="$ROOT_DIR/evidence/beta15_6-package"
OUT_DIR="$ROOT_DIR/evidence/beta15_6-package-smoke"
...
[[ "$manifest_decision" == "PASS_BRIK64_CLI_BETA15_6_PACKAGE_BUILT" ]] || { echo "package_decision_drift:$manifest_decision" >&2; exit 1; }
[[ "$release_eligible" == "true" ]] || { echo "beta15_6_public_package_should_be_release_eligible" >&2; exit 1; }
[[ "$(sha256_file "$package_path")" == "$package_sha" ]] || { echo "package_hash_mismatch" >&2; exit 1; }

jq -e '.decision == "PASS_BRIK64_CLI_BETA15_6_PRE_PUBLIC_RC_GATE"' "$ROOT_DIR/evidence/beta15_6-pre-public-rc/report.json" >/dev/null || {
  echo "external_beta15_6_pre_public_gate_not_pass" >&2
  exit 1
}
jq -e '.decision == "PASS_BRIK64_CLI_BETA15_6_RUST_F64_COMMAND_LIFT_GATE"' "$ROOT_DIR/evidence/beta15_6-rust-f64-command-lift/report.json" >/dev/null || {
  echo "external_beta15_6_rust_f64_command_lift_gate_not_pass" >&2
  exit 1
}
...
[[ ! -e "$EXTRACTED/evidence/beta15_6-pre-public-rc/report.json" ]] || {
  echo "package_contains_mutable_beta15_6_pre_public_evidence" >&2
  exit 1
}
[[ -f "$EXTRACTED/pcd/beta15_6/cli/rust_f64_polymer_codegen.pcd" ]] || {
  echo "package_missing_beta15_6_pcd_contracts" >&2
  exit 1
}
[[ -f "$EXTRACTED/pcd/beta15_6/harness/lift_roundtrip_gate.pcd" ]] || {
  echo "package_missing_beta15_6_lift_roundtrip_contract" >&2
  exit 1
}
[[ -f "$EXTRACTED/pcd/beta15_6/release/public_surface_sync.pcd" ]] || {
  echo "package_missing_beta15_6_public_surface_contract" >&2
  exit 1
}
[[ ! -e "$EXTRACTED/evidence/beta15_6-rust-f64-command-lift/report.json" ]] || {
  echo "package_contains_mutable_beta15_6_rust_f64_command_lift_evidence" >&2
  exit 1
}
...
jq -n \
  --arg version "$VERSION" \
  '{
    schemaVersion:"brik64.cli_beta15_6_package_smoke_report.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA15_6_PACKAGE_SMOKE",
    releaseEligible:true,
    checked:[
      "package_sha",
      "external_beta15_6_evidence",
      "mutable_evidence_excluded_from_package",
      "embedded_beta15_6_rust_polymer_domain_gate",
      "version",
      "ledger_help",
      "certify",
...
printf 'decision=PASS_BRIK64_CLI_BETA15_6_PACKAGE_SMOKE\n'
