...
const evidenceDir = path.join(root, 'evidence', 'beta14_4-monomer-128');
...
record('pcd:source_scaffold_present', () => {
  const base = path.join(root, 'pcd', 'beta14_4');
  const required = [
    ...
    'release/beta14_4_release_train.pcd'
  ];
...
record('l6:prior_baseline_packaged_without_new_claim', () => {
  ...
  return { priorL6Baseline: 'beta14_3', claimBoundary: 'not_new_beta14_4_l6_materialization' };
});

fs.mkdirSync(evidenceDir, { recursive: true });
const report = {
  schemaVersion: 'brik64.cli_beta14_4_monomer_128_gate.v1',
  version,
  decision: failures.length === 0 ? 'PASS_BETA14_4_MONOMER_128_GATE' : 'FAIL_BETA14_4_MONOMER_128_GATE',
  evidenceLevel: failures.length === 0 ? 3 : 1,
  checks,
  failures,
  boundary: 'Registry, parser and source-scaffold gate only. Prior Beta14.3 L6 evidence is consumed as baseline; this is not fresh Beta14.4 L6 generation evidence, self-hosting, fixpoint, formal proof, or public release authorization.'
};
