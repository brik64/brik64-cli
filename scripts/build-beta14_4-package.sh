...
OUT_DIR="$ROOT_DIR/evidence/beta14_4-package"
...
audit_gate="$ROOT_DIR/evidence/beta14_4-audit-closure/report.json"
audit_gate_fallback="$ROOT_DIR/evidence/beta14_2-audit-closure/report.json"
monomer_gate="$ROOT_DIR/evidence/beta14_4-monomer-128/report.json"
executable_gate="$ROOT_DIR/evidence/beta14_4-128-executable/report.json"
...
[[ "$audit_gate_decision" == "PASS_BETA14_4_AUDIT_CLOSURE_GATE" || "$audit_gate_decision" == "PASS_BETA14_2_AUDIT_CLOSURE_GATE" ]] || failures+=("beta14_4_audit_closure_gate_missing_or_invalid:$audit_gate_decision")
[[ "$monomer_gate_decision" == "PASS_BETA14_4_MONOMER_128_GATE" ]] || failures+=("beta14_4_monomer_128_gate_missing_or_invalid:$monomer_gate_decision")
[[ "$executable_gate_decision" == "PASS_BETA14_4_128_EXECUTABLE_GATE" ]] || failures+=("beta14_4_128_executable_gate_missing_or_invalid:$executable_gate_decision")
...
if [[ "${#failures[@]}" -gt 0 ]]; then
  ...
  printf '%s\n' "${failures[@]}" | jq -Rsc \
    --arg version "$VERSION" \
    '{
      schemaVersion:"brik64.cli_beta14_4_package_manifest.v1",
      version:$version,
      decision:"FAIL_BRIK64_CLI_BETA14_4_PACKAGE_BUILT",
...
inputs=(
  ...
  "pcd/beta14_4"
  "evidence/beta14_4-monomer-128/report.json"
  "evidence/beta14_4-128-executable/report.json"
...
jq -n \
  ...
  '{
    ...
    version:$version,
    private:true,
	    description:"BRIK64 CLI beta14.4 candidate for local PCD workflows, 128 monomer registry inspection, lift previews, and secure evidence review.",
...
if [[ -e "$STAGE_DIR/evidence" ]]; then
  if [[ ! -e "$STAGE_DIR/evidence/beta14_4-monomer-128/report.json" ]]; then
...
jq -n \
  --arg version "$VERSION" \
  --arg packagePath "evidence/beta14_4-package/$PACKAGE_NAME" \
  --arg packageSha "$package_sha" \
  --argjson packageBytes "$(file_size "$PACKAGE_PATH")" \
  '{
    schemaVersion:"brik64.cli_beta14_4_package_manifest.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA14_4_PACKAGE_BUILT",
    releaseEligible:false,
	    lane:"cli_0_1_beta14_4",
      ...
      decision:"PASS_BETA14_4_MONOMER_128_GATE",
      report:"evidence/beta14_4-monomer-128/report.json",
      packaged:true
    },{
      decision:"PASS_BETA14_4_128_EXECUTABLE_GATE",
      report:"evidence/beta14_4-128-executable/report.json",
      packaged:true
    },{
      decision:"PASS_BETA14_3_L6_GENERATION_GATE_PRIOR_BASELINE",
      report:"evidence/beta14_3-l6-generation/gate-report.json",
      packaged:true,
      claimBoundary:"prior_baseline_not_new_beta14_4_l6_materialization"
    },{
      decision:"PASS_BETA14_4_AUDIT_CLOSURE_GATE_OR_PRIOR_BETA14_2_BASELINE",
      report:"evidence/beta14_2-audit-closure/report.json",
      packaged:false
    }],
    requiredPublicReleaseGates:[
      "beta14_4_package_smoke",
      "beta14_4_github_release",
      "curl_gcp_installer_beta14_4",
      "web_docs_changelog_beta14_4",
      "skills_beta14_4",
      "sdk_beta14_4_marketplaces_or_deferrals",
      "public_claim_scan",
      "live_release_train_verify"
    ],
	    boundary:"Beta14.4 local package candidate only. Public release remains blocked until package smoke, GitHub, curl/GCP, docs, web, changelog, SDK, skills, marketplace publication and live verification pass atomically. Prior Beta14.3 L6 evidence is packaged as baseline only, not as a fresh Beta14.4 L6 materialization claim."
  }' > "$MANIFEST_PATH"
...
printf 'decision=PASS_BRIK64_CLI_BETA14_4_PACKAGE_BUILT\n'
printf 'releaseEligible=false\n'
printf 'package=evidence/beta14_4-package/%s\n' "$PACKAGE_NAME"
printf 'sha256=%s\n' "$package_sha"
