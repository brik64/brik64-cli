...
OUT_DIR="$ROOT_DIR/evidence/beta15_2-package"
...
pre_public_gate="$ROOT_DIR/evidence/beta15_2-pre-public-rc/report.json"
...
[[ "$pre_public_gate_decision" == "PASS_BRIK64_CLI_BETA15_2_PRE_PUBLIC_RC_GATE" ]] || failures+=("beta15_2_pre_public_gate_missing_or_invalid:$pre_public_gate_decision")
...
if [[ "${#failures[@]}" -gt 0 ]]; then
  ...
  printf '%s\n' "${failures[@]}" | jq -Rsc \
    --arg version "$VERSION" \
    '{
      schemaVersion:"brik64.cli_beta15_2_package_manifest.v1",
      version:$version,
      decision:"FAIL_BRIK64_CLI_BETA15_2_PACKAGE_BUILT",
      releaseEligible:false,
      failures:(split("\n") | map(select(length > 0)))
    }' > "$MANIFEST_PATH"
  printf 'beta15_2_package_input_gate_failed:%s\n' "${failures[*]}" >&2
  exit 1
fi
...
jq -n \
  --arg version "$VERSION" \
  --arg packagePath "evidence/beta15_2-package/$PACKAGE_NAME" \
  --arg packageSha "$package_sha" \
  --argjson packageBytes "$(file_size "$PACKAGE_PATH")" \
  '{
    schemaVersion:"brik64.cli_beta15_2_package_manifest.v1",
    version:$version,
    decision:"PASS_BRIK64_CLI_BETA15_2_PACKAGE_BUILT",
    releaseEligible:false,
    lane:"cli_0_1_beta",
    ...
    requiredPublicReleaseGates:[
      "beta15_2_package_smoke",
      "github_release_beta15_2",
      "curl_gcp_installer_beta15_2",
      "web_docs_changelog_beta15_2",
      "skills_beta15_2",
      "sdk_beta15_2_marketplaces_or_deferrals",
      "public_claim_scan",
      "live_release_train_verify"
    ],
    boundary:"Beta15.2 local package candidate only. Public release remains blocked until package smoke, GitHub, curl/GCP, docs, web, changelog, SDK, skills, marketplace publication or deferral, and live verification pass atomically."
  }' > "$MANIFEST_PATH"
...
printf 'decision=PASS_BRIK64_CLI_BETA15_2_PACKAGE_BUILT\n'
printf 'releaseEligible=false\n'
printf 'package=evidence/beta15_2-package/%s\n' "$PACKAGE_NAME"
printf 'sha256=%s\n' "$package_sha"
