...
OUT_DIR="$ROOT_DIR/evidence/beta15_5-package"
...
pre_public_gate="$ROOT_DIR/evidence/beta15_5-pre-public-rc/report.json"
...
[[ "$pre_public_gate_decision" == "PASS_BRIK64_CLI_BETA15_5_PRE_PUBLIC_RC_GATE" ]] || failures+=("beta15_5_pre_public_gate_missing_or_invalid:$pre_public_gate_decision")
...
if [[ "${#failures[@]}" -gt 0 ]]; then
  ...
  printf '%s\n' "${failures[@]}" | jq -Rsc \
    ...
    '{
      ...
      version:$version,
      decision:"FAIL_BRIK64_CLI_BETA15_5_PACKAGE_BUILT",
      releaseEligible:false,
      failures:(split("\n") | map(select(length > 0)))
    }' > "$MANIFEST_PATH"
  printf 'beta15_5_package_input_gate_failed:%s\n' "${failures[*]}" >&2
  exit 1
fi
...
jq -n \
  --arg version "$VERSION" \
  --arg packagePath "evidence/beta15_5-package/$PACKAGE_NAME" \
  ...
  '{
    ...
    requiredPublicReleaseGates:[
      ...
      "github_release_beta15_5",
      "curl_gcp_installer_beta15_5",
      "web_docs_changelog_beta15_5",
      "skills_beta15_5",
      "sdk_beta15_5_marketplaces_or_deferrals",
      "public_claim_scan",
