...
} = require('./beta15_7-l6-materialization-result');
const {
  buildRequest,
  validateRequest
} = require('./beta15_7-l6-materializer-request-bundle');
...
function technicalSheet(sourceHash, sourcePath) {
  const base = {
    ...
    boundedDomains: {
      ...
      cli_scope_beta15_7: { kind: 'integer', min: 0, max: 1 },
    ...
    normalization: {
      status: 'finite_beta15_7_materialization_domains',
    ...
    certificationLane: 'INTERNAL_BETA15_7_L6_MATERIALIZATION',
...
function directRoute2Materialization(inputs, expectedContext, remoteRefs) {
  ...
  const packageRef = fileRef('evidence/beta15_7-package/brik64-cli-0.1.0-beta.15.7.tgz');
  const sealPath = 'evidence/beta15_7-l6-generation/seal_report.json';
  const generatedArtifactRef = fileRef('evidence/beta15_7-l6-generation/materialization-out.tgz');
  ...
  writeJson(path.join(root, sealPath), {
    ...
    version,
    decision: summary.failures === 0 ? 'PASS_BETA15_7_L6_SEAL' : 'BLOCKED_BETA15_7_L6_SEAL',
    compositeSha256,
    generationTraceSha256,
  ...
  return {
    schemaVersion: 'brik64.beta15_7_cli_l6_materialization_result.v1',
    version,
...
function main() {
  ...
  writeJson(path.join(evidenceDir, 'generated_artifact_manifest.json'), {
    ...
    requiredNextAction: materialization
      ...
      : 'add or expose a Beta15.7 CLI PCD/polymer materialization endpoint, then rerun generation from canonical PCD inputs',
  ...
  const gateReport = {
    ...
    nextAction: 'implement or expose L6+N5 CLI artifact materializer for PCD/polymer -> artifact -> package -> release manifest before Beta15.7 publication'
  };
